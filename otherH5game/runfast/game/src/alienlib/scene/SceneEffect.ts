...
module PDKalien.sceneEffect{
	...
	export class Material implements ISceneEffect{
		...
		handover(scene1:PDKalien.SceneBase, scene2:PDKalien.SceneBase, parent:eui.Group, effectParams:any, callback:Function):void {
			...
			mask.x = pos.x;
			mask.y = pos.y;
			PDKalien.StageProxy.root.addChild(mask);
			...
			egret.Tween.get(this).to({r: distance}, effectParams.duration || 1000, egret.Ease.quartOut).call(()=>{
				...
				parent.removeChild(scene1);
				scene2.mask = null;
				PDKalien.StageProxy.root.removeChild(mask);
