...
module PDKalien{
	export class ScrollNavigator extends eui.Group {
		...
		private setup():void{
			...
			PDKalien.PDKUtils.enumChildren(this, (child:egret.DisplayObject, index:number)=>{
			...
			PDKalien.StageProxy.stage.addEventListener(egret.TouchEvent.TOUCH_MOVE, this.onTouchMove, this, true);
			PDKalien.StageProxy.stage.addEventListener(egret.TouchEvent.TOUCH_END, this.onTouchEnd, this, true);
		...
		private onTouchEnd(event:egret.TouchEvent):void{
			if(this._touchBeginPoint){
				...
				if(Math.abs(this._dragOffset.x) > this.width / 3){
					page = this.switchPage(PDKalien.MathUtils.sign(-this._dragOffset.x));
				}else if(l > 0){
					...
					if(p){
						...
						//console.log(sx);
						if(Math.abs(sx) > 1){
							page = this.switchPage(PDKalien.MathUtils.sign(sx));
						}
					}
				}
