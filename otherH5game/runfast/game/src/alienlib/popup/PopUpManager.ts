...
module PDKalien{
	export class PopUpManager{
		...
		constructor(){
			this._popLayer = PDKSceneManager.instance.popLayer;
			this._popLayer.width = PDKalien.StageProxy.width;
			this._popLayer.height = PDKalien.StageProxy.height;
		...
		addPopUp(target:eui.Component, effectClazz:any = null, effectParams:any = null, modalTouchFun:Function = null, modal:boolean=true, modalConfig:any = null):void{
			...
			this.modalConfig = PDKalien.PDKUtils.combineProp([modalConfig, PopUpManager.defaultModalConfig]);
		...
		removeAllPupUp():void{
			PDKalien.PDKUtils.enumChildren(this._popLayer, (popup:any)=>{
				if(popup != this._modalMask){
					popup['close']();
				}
			});
		}
		...
		createEffectInstance(effectClazz:any = null):popupEffect.IDialogEffect{
			...
			if(effectClazz){
				...
				effect = new PDKalien.popupEffect.None();
			}
