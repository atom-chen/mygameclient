...
module PDKalien{
	export class Navigator extends eui.Group{
		...
		private _pageStack:PDKNavigationPage[];
		private _tween:egret.Tween;
		private _currentPage:PDKNavigationPage;
		...
		push(pageDef:any, animate:boolean = true, params:any = null, dispatch:boolean = true):void{
			...
			let page:PDKNavigationPage = this._pageCache[pageDefName];
			...
			if(this._pageStack.length > 0){
				let sw:number = PDKalien.StageProxy.width;

				let lastPage:PDKNavigationPage = this._pageStack[this._pageStack.length - 1];
		...
		pop(animate:boolean = true, params:any = null, dispatch:boolean = true):void{
			...
			if(this._pageStack.length > 1){
				...
				let gonePage:PDKNavigationPage = this._pageStack.pop();
				let page:PDKNavigationPage = this._currentPage = this._pageStack[this._pageStack.length - 1];
		...
		get currentPage():PDKNavigationPage{
			return this._currentPage;
		}
	}

	export class PDKNavigationPage extends eui.Component{
