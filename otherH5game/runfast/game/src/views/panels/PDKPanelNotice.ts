...
class PDKPanelNotice extends PDKalien.PDKPanelBase {
    private static _instance: PDKPanelNotice;
    ...
    init():void {
		this.skinName = panels.PDKPanelNoticeSkin;
	}
	...
	private _onClickCopyKeFuWX():void{
		let _copyString = PDKGameConfig.wxService;
		if(this.currentState == "delegate"){ //代理的微信号不同
			_copyString = PDKGameConfig.extendWX;
		}
	 ...
	 * 获取公告单例
	 */
    public static getInstance(): PDKPanelNotice {
        if(!PDKPanelNotice._instance) {
            PDKPanelNotice._instance = new PDKPanelNotice();
        }
        return PDKPanelNotice._instance;
    }
	...
	public static remove():void{
		if(PDKPanelNotice._instance){
			PDKPanelNotice._instance.close();
		}
	}
}