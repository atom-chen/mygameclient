...
class PDKPanelFirstRecharge extends PDKalien.PDKPanelBase {
    private static _instance: PDKPanelFirstRecharge;
	/**
	 * 正常的字
	 */
	 ...
	 * 去充值按钮
	 */
	private mGoRecharge_btn:PDKButtonDoTaskSkin;
	 ...
	 */
	init():void{
		this.skinName = panels.PDKPanelFirstRecharge;
	}
	...
	private _initGoRechargeBtn():void{
		...
		if(!_hasBuy){
			this.mGoRecharge_btn.setPngs("frecharge_gopay_n","frecharge_gopay_p");
			this.mGoRecharge_btn.setClickFunc(this._onGoRecharge.bind(this));
		}
		else{
			this.mGoRecharge_btn.setPngs("frecharge_get_n","frecharge_get_p");
	...
	private _playLightOnTarget(target:eui.Image):any{
		let mcData = RES.getRes("pdk_frLightAni_json");
		let pngData = RES.getRes("pdk_frLightAni_png");
	...
	private _onAddToStage():void{
        ...
        e.registerOnObject(this,PDKalien.Dispatcher,PDKEventNames.MY_USER_INFO_UPDATE,this._onMyUserInfoUpdate,this);
	...
	private _onRemoveFromStage():void{
		...
		PDKPanelFirstRecharge._instance = null;
	 ...
	 * 获取首充UI单例
	 */
    public static getInstance(): PDKPanelFirstRecharge {
        if(!PDKPanelFirstRecharge._instance) {
            PDKPanelFirstRecharge._instance = new PDKPanelFirstRecharge();
        }
        return PDKPanelFirstRecharge._instance;
    }
	...
	public static setDisableGet():void{
		if(PDKPanelFirstRecharge._instance) {
			PDKPanelFirstRecharge._instance.mGoRecharge_btn.visible = false;
		}
	}
	...
	public static remove():void{
		if(PDKPanelFirstRecharge._instance){
			PDKPanelFirstRecharge._instance.close();
		}
	}
