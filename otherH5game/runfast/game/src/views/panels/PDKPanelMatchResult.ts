...
class PDKPanelMatchResult extends PDKalien.PDKPanelBase {
	private static _instance: PDKPanelMatchResult;
	public static get instance(): PDKPanelMatchResult {
		if (this._instance == undefined) {
			this._instance = new PDKPanelMatchResult();
		}
	...
	protected init(): void {
		this.skinName = panels.PDKPanelMatchResultSkin;
		this.list.itemRenderer = PDKRankItem;
	...
	private onBtnCancelTap(event: egret.TouchEvent): void {
		if (this.rewardContent && this.rewardContent.length > 0) {
			if (PDKalien.Native.instance.isNative) {
				PDKAlert.show('领取奖励：' + this.rewardContent + ' 成功')//zhu 暂时屏蔽, 2, this.onAlertBtn);
			} else {
				PDKAlert.show('领取奖励：' + this.rewardContent + ' 成功');
			}
		}
		this.dealAction();
	}
	...
	private onAlertBtn(action: string, data: any): void {
		if (action == 'cancel') {
			...
			let shareScene: number = PDKShareImageManager.instance.getShareScene(PDKShareImageManager.GAME_TYPE_MATCH, code);
	...
	show(data: any, callback: Function): void {
		...
		this.labMatchName.text = PDKlang.format(PDKlang.id.match_result_match_name, pdkServer.roomInfo.name);
		let num: number = data.params[0];
		this.labNumber.text = num.toString();
		this.labPraise.text = PDKlang.match_result_praise[num > 3 ? 2 : (num > 1 ? 1 : 0)];
		let reward = "";
		if (data.strreward) {
			reward = this._formatRewInfo(data.strreward);
		} else {
			reward = PDKMatchService.getRewardStringByRank(pdkServer.roomInfo, num, '\n');
		}
...
class PDKRankItem extends eui.ItemRenderer {
	...
	protected dataChanged(): void {
		...
		if (PDKMainLogic.instance.selfData.uid == this.data.uid) {
