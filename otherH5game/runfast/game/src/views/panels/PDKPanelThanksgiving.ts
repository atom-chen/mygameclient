...
class PDKPanelThanksgiving extends PDKalien.PDKPanelBase {
    private static _instance: PDKPanelThanksgiving;
	/**
	 * 购买按钮
	 */
    ...
    init():void {
		this.skinName = panels.PDKPanelThanksgivingSkin;
	...
	private _initEvent():void{
		...
		this.getImg["addClickListener"](this._onClickGetImg,this);
		 ...
		 pdkServer.addEventListener(PDKEventNames.USER_OPERATE_REP,this._onRecvUserOperateRep,this);
		 pdkServer.addEventListener(PDKEventNames.USER_RECHARGE_RESULT_NOTIFY, this._onRechargeResultNotify, this);
	}
	...
	private _onClickBuyImg():void{
		...
		PDKRechargeService.instance.doRecharge(_productId);
	}
	...
	private _initSignInfo():void{
		...
		for(let i =0;i<this._signInfo.length;++i){
			if(this._signInfo[i] == 1){
				...
				this["get" + i + "Img"].source = "thanks_r";
			}else if(this._signInfo[i] == 0){
				this["get" + i + "Img"].visible = false;
			}else{
				this["get" + i + "Img"].visible = true;
				this["get" + i + "Img"].source = "thanks_w";
			}
		}
	}
	/**
	...
	private _onRemovedToStage():void{
		...
		PDKPanelThanksgiving._instance = null;
	}
	/**
	 * 使能 事件
	 */
	 ...
	 * 获取感恩节单例
	 */
    public static getInstance(): PDKPanelThanksgiving {
        if(!PDKPanelThanksgiving._instance) {
            PDKPanelThanksgiving._instance = new PDKPanelThanksgiving();
        }
        return PDKPanelThanksgiving._instance;
    }
	/**
	 * 移除感恩节面板
	 */
	public static remove():void{
		if(PDKPanelThanksgiving._instance){
			PDKPanelThanksgiving._instance.close();
		}
	}
}