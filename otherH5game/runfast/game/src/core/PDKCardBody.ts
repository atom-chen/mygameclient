...
class PDKCardBody extends egret.DisplayObjectContainer {
	...
	initData(pid):void {
		...
		if (pid > 0) {
			this.addChild(this._bg = new egret.Bitmap(RES.getRes('pdk_poker_broad')));

			let poker:any = this._poker = PDKUtils.pid2poker(pid);
			if (poker.num > 13) {
				let joker:egret.Bitmap = new egret.Bitmap(RES.getRes('pdk_poker_' + poker.signColor + '_joker'));
				this.addChild(joker);

				let icon:egret.Bitmap = new egret.Bitmap(RES.getRes('pdk_poker_' + poker.signColor + '_joker_ddz_icon'));
				icon.x = 0;
				icon.y = 0;
				this.addChild(icon);
			} else {
				...
				let num:egret.Bitmap = new egret.Bitmap(RES.getRes('pdk_poker_' + poker.signColor + '_' + numStr));
				this.addChild(num);
				let signIcon:egret.Bitmap = new egret.Bitmap(RES.getRes('pdk_poker_sign_' + poker.sign));
				this.addChild(signIcon);

				num = new egret.Bitmap(RES.getRes('pdk_poker_' + poker.signColor + '_' + numStr));
				num.anchorOffsetX = num.x = num.width / 2;
				num.anchorOffsetY = num.y = num.height / 2;
				num.rotation = 180;
				this.addChild(num);
				signIcon = new egret.Bitmap(RES.getRes('pdk_poker_sign_' + poker.sign));
			...
			this._masterFlag = new egret.Bitmap(RES.getRes('pdk_icon_master_card'));
	...
	set rollOver(value:boolean) {
		...
		if (this._rollOver != value) {
			...
			this._bg.texture = RES.getRes('pdk_poker_broad' + (this._rollOver ? '_h' : ''));
		}
	}
