...
module CCalien{
	export class CCDDZNavigator extends eui.Group{
		...
		push(pageDef:any, animate:boolean = true, params:any = null, dispatch:boolean = true):void{
			...
			if(dispatch){
				this.dispatchEventWith(CCDDZNavigator.NAVIGATE, false, {action, page});
			}

			if(this._pageStack.length > 0){
				let sw:number = CCalien.CCDDZStageProxy.width;
				...
				if(animate){
					egret.Tween.get(lastPage).to({x: -sw}, CCDDZNavigator.duration, egret.Ease.quartInOut).call(onLastPageHide, this);
				}else{
					lastPage.x = -sw;
				...
				if(animate){
					this._tween = egret.Tween.get(page).to({x: 0}, CCDDZNavigator.duration, egret.Ease.quartInOut).call(onPageShow, this);
				}else{
					page.x = 0;
		...
		pop(animate:boolean = true, params:any = null, dispatch:boolean = true):void{
			...
			if(this._pageStack.length > 1){
				...
				page.beforeShow(action, params);
				if(animate){
					egret.Tween.get(page).to({x: 0}, CCDDZNavigator.duration, egret.Ease.quartInOut).call(onPageShow, this);
				}else{
					page.x = 0;
					onPageShow.call(this);
				}
				...
				if(animate){
					this._tween = egret.Tween.get(gonePage).to({x: sw}, CCDDZNavigator.duration, egret.Ease.quartInOut).call(onGonePageHide, this);
				}else{
					gonePage.x = sw;
		...
		popAll(animated:boolean = true, params:any = null, dispatch:boolean = true):void{
			...
			if(this._pageStack.length > 1){
				...
				gonePage.beforeHide(action, params);
				if(animated){
					egret.Tween.get(page).to({x: 0}, CCDDZNavigator.duration, egret.Ease.quartInOut).call(()=>{
						page.onShow(action, params);
					});

					this._tween = egret.Tween.get(gonePage).to({x: sw}, CCDDZNavigator.duration, egret.Ease.quartInOut).call(()=>{
						gonePage.onHide(action, params);
						this.removeChild(gonePage);
						this._tween = null;
					});
		...
		active(dispatch:boolean = true):void{
			if(this._needActive){
				...
				if(dispatch){
					...
					this.dispatchEventWith(CCDDZNavigator.NAVIGATE, false, {action: 'pop', page: this._currentPage});
				}
			}
		}
