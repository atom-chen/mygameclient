...
class CCDDZDDZCardGroup extends CCDDZCardGroup {
	...
	private onTouchEnd(event: egret.TouchEvent): void {
		if (this._touchBegin) {
			...
			for (let i = this._selectFrom; i <= this._selectTo; i++) {
				let card: CCDDZCard = <CCDDZCard>this.getChildAt(i);
				if (card) {
					card.rollOver = false;
					cards.push(card);
				}
	 ...
	 * @param cards
	 */
	selectCards(cards: CCDDZCard[]): void {
		...
		if (allselected) {
			CCalien.CCDDZUtils.enumChildren(this, (card: CCDDZCard, index: number) => {
		...
		// 	arrSelect = CCDDZCardsHint.getFitCards(_cardPokerIds,_selectIdx,this._targetCards);
		// //本轮是否是我自己出首牌
		// }else 
	...
	selectCard(card: CCDDZCard, select: boolean): void {
	 ...
	 * @param updateCount
	 */
	useCards(pokerIds: number[], receive: CCDDZDDZCardGroup, updateCount: boolean = true, isReconnect: boolean = false): number {
		let cards: CCDDZCard[] = [];
		if (this.currentState != "my") {
			if (this.currentState == "right_2") {
				pokerIds.forEach((pid: number, idx: number) => {
					...
					this.addChild(card);
					card.sending = true;
					let card1: CCDDZCard = <CCDDZCard>this.getChildAt(idx);
					let x = 0;
					let y = 0;
					if (card1) {
						x = card1.x;
...
window["CCDDZDDZCardGroup"] = CCDDZDDZCardGroup;