...
class CCDDZMySeat extends CCDDZDDZSeat {
	...
	addCards(cardids:number[], animation:boolean = true, cleancards:boolean = false):void {
		...
		this.ddzHandCardGroup.updateMyGrayCards();
	}

	useCard(pokerIds:number[] = null, gameEnd:boolean = false, updateCount:boolean = true,isReconnect:boolean = false):number {
		let cnt = super.useCard(pokerIds,gameEnd,updateCount,isReconnect);
		/*只有是我直接出牌才更新我被让的牌 
		* 其他玩家出牌是从左往右移除的不用更新被让的牌
		*/
		this.ddzHandCardGroup.updateMyGrayCards();
		return cnt;
	}
...
window["CCDDZMySeat"]=CCDDZMySeat;