...
class CCDDZDDZSeat extends CCDDZSeat {
	protected container: eui.Group;
	protected cdClock: CCDDZCDClock;
	protected labCardCount: eui.Label;
	protected imgMaster: eui.Image;
	protected robot: CCDDZRobot;
	protected imgStatus: eui.Image;
	protected imgReady: eui.Image;
	protected jingdengGroup: eui.Group;

	protected ddzHandCardGroup: CCDDZDDZCardGroup;
	protected ddzDeskCardGroup: CCDDZDDZCardGroup;
	...
	protected createChildren(): void {
		...
		this.ddzHandCardGroup = <CCDDZDDZCardGroup>this.handCardGroup;
		this.ddzDeskCardGroup = <CCDDZDDZCardGroup>this.deskCardGroup;
	...
	clean(keepCards: boolean = false, cleanUserInfo: boolean = true): CCDDZSeat {
	...
	protected onCardAdded(card: CCDDZCard): void {
	...
	protected setCardsShadowSize(cardcount): void {
		if (this.cardShadow) {
			this.cardShadow.width = CCGlobalGameConfig.CARD_WIDTH + (cardcount - 1) * CCGlobalGameConfig.CARD_GAP_H;
		}
	}
	...
	useCard(pokerIds: number[] = null, gameEnd: boolean = false, updateCount: boolean = true, isReconnect: boolean = false): number {
		...
		if (!gameEnd && updateCount) {
			...
			let result = CCDDZCardsType.GetType(pokerIds);
			cardType = result.ct;
			let poker: any = CCDDZUtils.pid2poker(pokerIds[0]);
			CCDDZGameSoundManager.playCardType(this._sexVoice, cardType, poker.num);
			CCalien.CCDDZDispatcher.dispatch(CCGlobalEventNames.SHOW_GAME_EFFECT, { cardType, state: this.currentState });

			//			switch(cardType){
			//    			//王炸
			//    			case 1:
			//    			    CCalien.CCDDZDispatcher.dispatch(CCGlobalEventNames.SHOW_GAME_EFFECT,cardType);
			//    			
			//				case 9: //飞机特殊
			//					break;
			//			}
		...
		if (pokerIds.length == 0) {
			...
			if (gameEnd) {
				...
				this.setStatus(CCGlobalResNames.cc_play_word_4);
			}
		} else {
			this.cleanDesk();
	...
	showScore(score: number): void {
		if (score == 0) {
			this.setStatus(CCGlobalResNames.cc_play_word_5);
		} else if (score < 0) {
			//this.imgStatus.source = CCGlobalResNames.cc_play_word_4;
		} else {
			this.setStatus('cc_play_word_' + score);
		}

		CCDDZGameSoundManager.playScore(this._sexVoice, score);
	}
	...
	showGrab(score: number): void {
		if (score == 0) {
			this.setStatus("cc_play_word_not_grab");
		}
		else {
			this.setStatus("cc_play_word_grab");
		}
		CCDDZGameSoundManager.playScore(this._sexVoice, score);
	}
	...
	showDoubleTip(bShow: boolean, bDouble: boolean = false): void {
		...
		egret.setTimeout(() => { //显示完不加倍或者是加倍后加一个自动隐藏的判断
			if (this.imgStatus.source == "cc_noDouble2" || this.imgStatus.source == "cc_double2") {
				this.setStatus();
			}
		}, this, 3000);
		CCDDZGameSoundManager.playDouble(this._sexVoice, bDouble);
	}
	...
	private jingdeng: CCDDZDragonJingDeng;
	public showJingDeng(): void {
		if (!this.jingdeng)
			this.jingdeng = new CCDDZDragonJingDeng();
	...
	setMaster(value: boolean, animation: boolean): void {
		...
		if (value) {
			let from: any = this.globalToLocal(CCalien.CCDDZStageProxy.width / 2 - this.imgMaster.width / 2, CCalien.CCDDZStageProxy.height / 2 - this.imgMaster.height / 2);
			from = {
				x: from.x,
				y: from.y,
				visible: true,
	...
	setReady(ready: boolean): void {
		this.imgReady.visible = ready && !ccserver.isMatch;
	}
}