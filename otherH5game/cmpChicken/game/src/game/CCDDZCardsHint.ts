...
class CCDDZCardsHint {
	...
	static getCardsHash(cards:Array<number>):{}{
		...
		for(var i = 0; i < cards.length; ++i){
			key = CCDDZCardsHint.getCardLogicValue(cards[i]);
			if(!hash[key]){
				hash[key] = [cards[i]];
			}else{
	...
	static getAllFourCard(cards:Array<number>, max_amount:number = 0){
		...
		for(var k in hasTab){
			if(hasTab[k].length == 4){
				if(max_amount < 8){
					var combs = this.getMinDouble(cards, 4, hasTab[k])
					if(combs && combs.length == 4){
						var tc = hasTab[k].concat(combs);
						ans = {node:CCDDZCardsType.GetType(tc), cards:tc};
				...
				if(max_amount < 4){
					ans = {node:CCDDZCardsType.GetType(hasTab[k]), cards:hasTab[k]};
					max_amount = 4;
				}
			}
		}
			...
			for(var j = i + 1; j < 13; ++j){
				if(hasTab[j]){
					...
					if(j == 12 && line.length > 4 && line.length > max_amount){
						ans = {node:CCDDZCardsType.GetType(line), cards:line};
	...
	static getAllCardsPossible(cards:Array<number>):any{
		...
		if(ct_amount[3] > 0 || ct_amount[4] > 0){
			var plane = this.getAllPlaneCard(cards, max_amount);
			if(plane){
				result = plane;
	...
	static getMaxAmountCards(cards:Array<number>):any{
		...
		var temp = CCDDZCardsType.GetType(cards)
		if(temp.ct > 0){
			return cards;
		}
		
		temp = CCDDZCardsHint.getAllCardsPossible(cards);
...
// let CCDDZSANSHUN:number = 12;
	 ...
	 static getFitCards(cards:Array<number>, idx:number, precards:Array<number> = []):any{
		...
		if(precards.length > 0){
			prenode = CCDDZCardsType.GetType(precards);
