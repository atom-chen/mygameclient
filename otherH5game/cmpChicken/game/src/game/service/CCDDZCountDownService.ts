...
class CCDDZCountDownService extends CCService{
	private static _instance:CCDDZCountDownService;
	public static get instance():CCDDZCountDownService {
		if (this._instance == undefined) {
			this._instance = new CCDDZCountDownService();
		}
	...
	private onTimer():void{
		...
		this._items.forEach((item:any, index:number)=> {
			let time:number = item.end - ccserver.tsServer;
	...
	static register(second:number, onSecond:Function, onComplete:Function = null):number{
		return CCDDZCountDownService.instance.register(second, onSecond, onComplete);
	}

	static unregister(id:number):void{
		CCDDZCountDownService.instance.unregister(id);
	}
}