...
class CCDDZMailService extends CCService {
	private static _instance: CCDDZMailService;
	public static get instance(): CCDDZMailService {
		if (this._instance == undefined) {
			this._instance = new CCDDZMailService();
		}
	...
	protected init(): void {
		...
		//e.register(name, ccserver, CCGlobalEventNames.USER_MAIL_LIST_REP, this.onMailListResponse, this);
		//e.register(name, ccserver, CCGlobalEventNames.USER_MAIL_DETAIL, this.onMailNotify, this);
		//e.register(name, ccserver, CCGlobalEventNames.USER_MAIL_MODIFY_REP, this.onMailModifyResponse, this);
		e.register(name, ccserver, CCGlobalEventNames.USER_MailGetReward_REP, this.onMailGetRewardResponse, this);
		//e.register(name, ccserver, CCGlobalEventNames.USER_MAIL_UNREAD_REP, this.onMailUnreadResponse, this);
	...
	start(cb): void {
		...
		this.getMailList();
		CCalien.CCDDZEventManager.instance.enable(egret.getQualifiedClassName(this));
	...
	stop(): void {
		CCalien.CCDDZEventManager.instance.disable(egret.getQualifiedClassName(this));
	...
	private callMailApi(action, callback, params = null, method = 'get') {
		...
		ccddzwebService.callApi('msg', action, callback, params, method)
	}
	 ...
	 */
	shouldLoadMore(){
		//console.log("CCDDZMailService shouldLoadMore===========>",this._haveMore,this._curPage,this._totalPage);
		if(this._haveMore){
	...
	getMailList(page=0) {
		...
		this.callMailApi('index', (response: any)=> {
			...
			if (response.code == 0) {
				...
				this.dispatchEventWith(CCGlobalEventNames.MAIL_LIST_REFRESH, false);
			}
		}, params)
	}
		...
		for(let i =0;i< _unReadList.length;++i){
			if(_unReadList[i].type == 4){//弹窗邮件
				if(_unReadList[i].status == 0){
					...
					CCDDZPanelMail.getInstance().tipOpenMailItem(_unReadList[i]);
					return;
				}
	...
	/**
	 * 如果是有附件的邮件，不要走此接口打开邮件，直接用CCDDZPanelMailDetails.getInstance().show(mailData);
	 */
	openMail(id: number) {
	...
	private addMailItem(item: any,hasOpen:boolean = false): void {
		...
		if (typeof item.attachment == 'string') {
			item.attachment = CCDDZUtils.parseGoodsString(item.attachment);
		}
		if(item.status !=0){
			hasOpen = true;
		}
	...
	private onMailGetRewardResponse(event: egret.Event): void {
		...
		if (data.result == 0) {
			...
			if (mailData) {
				...
				this.addMailItem(mailData, true);
				this.dispatchEventWith(CCGlobalEventNames.MAIL_LIST_REFRESH, false);
				CCDDZBagService.instance.refreshBagInfo(); //领取附件成功更新背包
				CCDDZPanelMailDetails.getInstance().close();
			}
		}else{
			CCDDZToast.show("领取失败！");
		}
	}
	...
	private updateUnreadCount(value: number = 0, dispatch: boolean = true): void {
		...
		if (dispatch) {
			this.dispatchEventWith(CCGlobalEventNames.MAIL_UNREAD_UPDATE, false, {count: this._unreadCount});
		}
	}
