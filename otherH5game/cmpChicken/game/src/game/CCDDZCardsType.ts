...
let CCDDZBUYAO:number = 0;
let CCDDZSHUANGWANG:number = 1;
let CCDDZZHADAN:number = 2;
let CCDDZDAN:number = 4;
let CCDDZDUIZI:number = 5;
let CCDDZSANZHANG:number = 6;
let CCDDZSANDAIYI:number = 7;
let CCDDZSANDAIYIDUI:number = 13;
let CCDDZSIDAIER:number = 8;
let CCDDZSIDAIERDUI:number = 14;
let CCDDZFEIJI:number = 9;
let CCDDZDANSHUN:number = 10;
let CCDDZSHUANGSHUN:number = 11;
let CCDDZSANSHUN:number = 12;

class CCDDZCardsType {
	static CheckType(cards:any[], targetCards:any[]):boolean {
		
		let result1 = this.GetType(cards);
		if (result1.ct == CCDDZSHUANGWANG) return true;
		...
		}else if(result2.ct != CCDDZSHUANGWANG){
			if(result2.ct != CCDDZZHADAN && result1.ct == CCDDZZHADAN){
				return true;
			}
		}
...
//				{chai:false,cards:GetCardsByNum([i],totalCards)};
	...
	static m_getCards(targetCards:any[], type:number, out1Arr:any[], out2Arr:any[], out3Arr:any[], out4Arr:any[], totalCards:any[], targetType:number):Object {
		...
		if (type == CCDDZSHUANGWANG || type == 0) return null;
	...
	static  GetType(cards:any[], out1Arr:any[] = null, out2Arr:any[] = null, out3Arr:any[] = null, out4Arr:any[] = null):any {
		...
		if (cards.length == 1) {
			return {ct:CCDDZDAN, kv:tmp1Arr[0]};
		}
		if (cards.length == 2 && cards.indexOf(140) >= 0 && cards.indexOf(150) >= 0) {
			return {ct:CCDDZSHUANGWANG, kv:15};
		}

		//炸弹
		if (tmp4Arr.length == 1 && cards.length == 4) {
			return {ct:CCDDZZHADAN, kv:tmp4Arr[0]};
		}
		//对子
		if (tmp2Arr.length == 1 && cards.length == 2) {
			return {ct:CCDDZDUIZI, kv:tmp2Arr[0]};
		}
		//三张
		if (tmp3Arr.length == 1 && cards.length == 3) {
			return {ct:CCDDZSANZHANG, kv:tmp3Arr[0]};
		}
		//三带一
		if (tmp3Arr.length == 1 && cards.length == 4) {
			return {ct:CCDDZSANDAIYI, kv:tmp3Arr[0]};
		}
		//三带对
		if (tmp3Arr.length == 1 && tmp2Arr.length == 1 && cards.length == 5) {
			return {ct:CCDDZSANDAIYIDUI, kv:tmp3Arr[0]};
		}
		//四带二
		if (tmp4Arr.length == 1 && cards.length == 6){
			return {ct:CCDDZSIDAIER, kv:tmp4Arr[0]};
		}
		//四带对
		if(cards.length == 8){
			if(tmp4Arr.length == 1 && tmp2Arr.length == 2){
				return {ct:CCDDZSIDAIERDUI, kv:tmp4Arr[0]};
			}else if(tmp4Arr.length == 2){
				return {ct:CCDDZSIDAIERDUI, kv:tmp4Arr[1]};
			}
		}
		//飞机
		if(tmp3ArrX.length > 1){
			...
			if(tmp.result){
				if(cards.length == 4 * (tmp3ArrX.length - 1)){ //飞机带单
					return {ct:CCDDZFEIJI, kv:tmp.kv};
				}
			}
		...
		if(tmp3Arr.length > 1){
			...
			if(tmp.result){
				...
				if(tmp3Arr.length * 3 == cards.length){
					return {ct:CCDDZSANSHUN, kv:tmp.kv};
				}
			}
		}
		//单顺
		if(tmp1Arr.length > 4 && cards.length == tmp1Arr.length) {
			...
			if(tmp.result){
				return {ct:CCDDZDANSHUN, kv:tmp.kv};
			}
		}
		...
		if(tmp2Arr.length > 2 && tmp2Arr.length * 2 == cards.length) {
			...
			if(tmp.result){
				return {ct:CCDDZSHUANGSHUN, kv:tmp.kv};
			}
		}
