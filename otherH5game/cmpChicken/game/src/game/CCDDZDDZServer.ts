...
class CCDDZDDZServer extends CCServer{
	protected beforeDispatchMessage(name:string, message:any):void {
		switch (name) {  //一些特殊处理的协议
			case CCGlobalEventNames.GAME_SET_SESSION:
				...
				this.resetSession();
				break;
			case CCGlobalEventNames.GAME_GAME_START_NTF:
	...
	setCards(cardids):void {
		this.send(CCGlobalEventNames.GAME_SET_CARDS, {
				seatid: 0,
				cardids: cardids,
			});
	}
	...
	quickJoin(flag:number =0):void {
		this.send(CCGlobalEventNames.USER_QUICK_JOIN_REQUEST, {roomid: this.roomInfo.roomID, clientid: this._clientId,flag:flag});
	}
	...
	userInfoRequest(uid:number):void {
		this.send(CCGlobalEventNames.USER_USER_INFO_REQUEST, {uid});
	}
	...
	getUserInfoInGame(uid:number):void {
		this.send(CCGlobalEventNames.GAME_USER_INFO_IN_GAME_REQ, {uid});
	}
	
    CreatePersonalGameReq(gameid: number,bscore: number,maxround: number,maxodd:number,minscore:number, cost: number = 0): void {
        this.send(CCGlobalEventNames.CREATE_PERSONAL_GAME_REQ,{ gameid:gameid,bscore:bscore,maxround:maxround, maxodd:maxodd, minscore:minscore, cost:cost});
    }
    
    JoinPGameReq(roomid: number): void {
        this.send(CCGlobalEventNames.JOIN_PGAME_REQ,{ roomid: roomid});
    }
    
    StartPGameReq(roomid: number): void {
        this.send(CCGlobalEventNames.START_PGAME_REQ,{ roomid: roomid });
    }
    
    QueryRoomInfoReq(roomid: number): void {
        this.send(CCGlobalEventNames.QUERY_ROOMINFO_REQ,{ roomid: roomid });
    }
    
    QuitPGameReq(roomid: number): void {
        this.send(CCGlobalEventNames.QUIT_PGAME_REQ,{ roomid: roomid });
    }
    
    GameDissolve(roomid: number): void {
        this.send(CCGlobalEventNames.GAME_DISSOLVE,{ roomid: roomid });
    }
    
    ReconnectPGameReq(): void {
        this.send(CCGlobalEventNames.RECONNECT_PGAME_REQ,{});
    }
...
//    static RECONNECT_PGAME_REQ: string = "user.ReconnectPGameReq";
	...
	answerMaster(score:number):void {
		this.send(CCGlobalEventNames.GAME_ANSWER_MASTER, {
			score,
		})
	}
	...
	useCardNtf(cardids:number[]):void {
		this.send(CCGlobalEventNames.GAME_USE_CARD_NTF, {
			seatid: this.seatid,
			cardids,
			index: 0,
		});
	...
	talk(content:string):void {
		this.send(CCGlobalEventNames.GAME_TALK_NTF, {
			seatid: 0,
			msg: content
		});
	}
	...
	hang(hang:boolean):void {
		this.send(CCGlobalEventNames.GAME_OPERATE_REQ, {
			optype: hang ? 1 : 0,
		});
	}
	...
	getPlayersAmount(gameId:number):void {
		this.send(CCGlobalEventNames.USER_PLAYERS_AMOUNT_REQ, {
			game_id: gameId,
		});
	}
	...
	modifyUserInfo(data:any):void {
		this.send(CCGlobalEventNames.USER_MODIFY_USER_INFO_REQ, data);
	}
	 ...
	 */
	giveUpGame():void{
		this.send(CCGlobalEventNames.GAME_GIVE_UP_GAME_REQ, {
			status: 0
		});
	}
	...
	QuitWaitingQueueReq():void{
		this.send(CCGlobalEventNames.QUIT_WAITING_QUEUE_REQ, {});
	}
	...
	getMatchSignUpInfo(matchid:number):void{
		this.send(CCGlobalEventNames.USER_MATCH_SING_UP_INFO_REQ, {matchid});
	}
	...
	matchSignUp(matchid:number, optype:number, itemid:number = 0):void{
		this.send(CCGlobalEventNames.USER_MATCH_SING_UP_REQ, {matchid, optype, itemid});
	}
	...
	enterMatch(matchid:number):void{
		this.send(CCGlobalEventNames.USER_ENTER_MATCH_REQ, {
			matchid,
	...
	giveUpMatch(matchid:number):void{
		this.send(CCGlobalEventNames.USER_GIVE_UP_MATCH_REQ, {
			matchid,
			clientid: this._clientId,
		});
	}
	 ...
	 */
	getBagInfo():void{
		this.send(CCGlobalEventNames.USER_BAG_INFO_REQ, {});
	}
	...
	GetFreshManReward():void{
		this.send(CCGlobalEventNames.GET_FRESHMAN_REWARD_REQ, {});
	}
	 ...
	 */
	alms():void{
		this.send(CCGlobalEventNames.USER_ALMS_REQ, {});
	}
	 ...
	 */
	getReady():void{
		this.send(CCGlobalEventNames.GAME_GET_READY_REQ, {});
	}
	/**
	 * 发送红包兑换请求
	 */
    redcoinExchangeGoodsReq(idx:number): void {
        this.send(CCGlobalEventNames.USER_REDCOIN_EXCHANGE_GOODS_REQ, {index:idx});
    }
	/**
	 * 钻石兑换请求
	 */
    ...
    lotteryRedCoinReq(roomId: number): void {
		let info = CCGlobalGameConfig.getRoomCfgFromAll(roomId);
		if(info){
			if(info.poolsupport == 1){
        		this.send(CCGlobalEventNames.USER_LOTTERY_RED_COIN_REQ, { roomid: roomId ,optype:1});
			}else if(info.rewardwinround == 1){
        		this.send(CCGlobalEventNames.USER_LOTTERY_RED_COIN_REQ, { roomid: roomId ,optype:4});
			}else if(info.style == "diamond"){
        		this.send(CCGlobalEventNames.USER_LOTTERY_RED_COIN_REQ, { roomid: roomId ,optype:2});
			}else{
				this.send(CCGlobalEventNames.USER_LOTTERY_RED_COIN_REQ, { roomid: roomId });
			}
		}else{
			CCDDZReportor.instance.reportCodeError("--------lotteryRedCoinReq-----error-->roomId:"+roomId);
		}
		...
		/*if(roomid == 1001||roomid == 1002 ||roomid == 8001){
        	...
        	this.send(CCGlobalEventNames.USER_LOTTERY_RED_COIN_REQ, { roomid: roomid ,optype:3});
		}else{
        	this.send(CCGlobalEventNames.USER_LOTTERY_RED_COIN_REQ, { roomid: roomid });
		}*/
    }
	 ...
	 */
    getSignReward(): void {
        this.send(CCGlobalEventNames.USER_DAY_SIGN_IN_REQ,{ optype: 2 });
    }
	 ...
	 */
    getSignInInfo(): void {
        this.send(CCGlobalEventNames.USER_DAY_SIGN_IN_REQ,{ optype: 1 });
    }
    ...
    {
		ccddzwebService.getRanklist();
        //this.send(CCGlobalEventNames.USER_REDCOIN_RANKING_LIST_REQ,{});
    }
    
	getCardsRecorderReward():void{
        this.send(CCGlobalEventNames.GET_CARDS_RECORDER_REWARD_REQ, {});
    }
	...
	public sendHornMsg(_type:number,_msg:string):void{
        ...
        this.send(CCGlobalEventNames.USER_PAY_TO_CHAT_REQ,data);
	}
	...
	public reqNewGoldRedRew():void{
        ...
        ccserver.send(CCGlobalEventNames.USER_LOTTERY_RED_COIN_REQ,data);
	}
	...
	public reqNationalActInfo():void{
        ...
        this.send(CCGlobalEventNames.USER_GET_ACTInfo_REQ,data);
	}
	...
	public reqDownAppRewInfo():void{
        let data = { optype: 1};
        ccserver.send(CCGlobalEventNames.USER_GET_ACTInfo_REQ,data);
	}
	/**
	 * 加倍 1不加倍 2加倍
	 */
	public reqDouble():void{
        ...
        ccserver.send(CCGlobalEventNames.GAME_OPERATE_REQ,data);
	...
	public reqUserOperate(optype,params):void{
		...
		ccserver.send(CCGlobalEventNames.USER_OPERATE_REQ,data);
	}
	...
	public reqRedNormalTime():void{
		let data = {optype:2};
		ccserver.send(CCGlobalEventNames.USER_DiamondGame_REQ,data);
	}
	...
	public reqMailOperate(optype:number,id:number):void{
		...
		ccserver.send(CCGlobalEventNames.USER_MailOperate_REQ,data);
	}
	...
	public reqMatchOperate(optype:number,matchid:number):void{
		...
		this.send(CCGlobalEventNames.USER_MatchOperate_REQ,_data);
	}
	...
	public reqMatchRevive():void{
		this.reqMatchOperate(1,ccserver.roomInfo.matchId);
	}
	...
	public reqAllLotteryStatus():void{
		let data = {optype:1}
		ccserver.send(CCGlobalEventNames.USER_CoinLottery_REQ,data);
	}
	...
	public isInNewyearTaskReqNewYear():void{
		//let isInNewYear = CCGlobalGameConfig.isInNewYearTask();
		//if(isInNewYear.isInTime){
			this.reqUserOperate(4,[3]);
		//}
	}
	...
	public isInNewyearWorkReqNewYear():void{
		let isInNewYear = CCGlobalGameConfig.isInNewYearWork();
		if(isInNewYear.isInTime){
	...
	public reqPlayerPraise(seatid:number,op:number):void{
		ccserver.gameOperate(8,[seatid,op]);
	}
...
let ccserver = new CCDDZDDZServer();