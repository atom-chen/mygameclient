...
class CCDDZGold extends eui.Component {
    ...
    //显示钻石
    showDiamond():void{
        this.imgIcon.source = "cc_icon_diamond";
    }
    ...
    setType(isMatch: boolean,matchType: number = 0): void {
        if(this._isMatch = isMatch) {
            switch(matchType) {
                case 0:   //积分赛
                    this.imgIcon.source = CCGlobalResNames.cc_icon_score;
                    break;
                case 1:   //金豆赛
                    this.imgIcon.source = CCGlobalResNames.cc_icon_gold;
                    break;
            }
        } else {
            this.imgIcon.source = CCGlobalResNames.cc_icon_gold;
