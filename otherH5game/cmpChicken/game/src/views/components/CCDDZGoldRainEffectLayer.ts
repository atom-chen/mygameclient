...
class CCDDZGoldRainEffectLayer extends eui.Group {
	...
	play():void{
		if(!this._goldItems){
			this._goldItems = [];
			let factory:egret.MovieClipDataFactory = new egret.MovieClipDataFactory(RES.getRes(CCGlobalResNames.ani_coin_json), RES.getRes(CCGlobalResNames.ani_coin_png));
			for(let i = 0; i < 35; i++){
		...
		CCalien.CCDDZSoundManager.instance.playEffect(CCGlobalResNames.effect_gold_rain);
	}
}
...
window["CCDDZGoldRainEffectLayer"]=CCDDZGoldRainEffectLayer;