 ...
 *
 */
class CCDDZMarqueeText extends eui.Component {
    public constructor() {
        super();
    }
...
//        CCDDZDispatcher.addListener(CCGlobalEventNames.CLEAR_LABA,this.clear,this);
        this.skinName = components.CCDDZMarqueeSkin;
//        this.txt.mask = this.maskR;
    ...
    protected addToStage(e: egret.Event): void {       
        this.x = CCalien.CCDDZStageProxy.stage.stageWidth - this.width >> 1;
        this.y=114;
...
//        this.addEventListener(egret.TouchEvent.TOUCH_ROLL_OUT,this.onMouseOut,this);
    ...
    private deleteAnnounce():void{
        CCDDZExchangeService.instance.deleteAnnounce();
    ...
    private _showOneMsgByChouRec(data:any):void{
        ...
        let name = "金豆场";
        if(data.room_id){
            name = lang.exchange_roomid[data.room_id];
        } 
        if(!name){
            name = "金豆场";
        }
        _ret.text= "恭喜<font color='#F0FF00'>" + data.nickname.substr(0,9)+ "</font>在"+ name +"抽得<font color='#F0FF00'>" + CCDDZUtils.exchangeRatio(data.money,true) + "</font>" + " 奖杯";
    ...
    public showLast(type:number = 1):void{
        ...
        if(_exIns.announce && _exIns.announce.length > 0){
            ...
            let _data = CCDDZMainLogic.instance.selfData.getHornTalkRecList(); //用户喇叭消息
            if(_data.length > 0){
            ...
            let _data = CCDDZExchangeService.instance.getOneChouMaxInfo();
            if(_data){
    ...
    private getNextText(): any {
        ...
        if(_exIns.announce && _exIns.announce.length > 0){
            ...
            if(data){
                ...
                str += "恭喜<font color='#F0FF00'>" + data.nickname.substr(0,9)+ "</font>在"+ name +"抽得<font color='#F0FF00'>" + CCDDZUtils.exchangeRatio(data.money,true) + "</font>" + "奖杯";
                _ret.text = str;
            }else{
                _ret.text = "";
            }
    ...
    public _onClickOpenHornTalk():void {
        CCDDZPanelHornTalk.getInstance().show();
    }
    ...
    private _getRankNext():void{
       let _infos = this["inviteRankList"] || [];
        ...
        if(_one){
            let _ret ={type:4,text:"",time:3000}; //CCDDZUtils.exchangeRatio(_one.coin / 100,true) 
...
window["CCDDZMarqueeText"]=CCDDZMarqueeText;