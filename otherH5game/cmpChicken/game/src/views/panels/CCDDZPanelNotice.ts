...
class CCDDZPanelNotice extends CCalien.CCDDZPanelBase {
    private static _instance: CCDDZPanelNotice;
    ...
    init():void {
		this.skinName = panels.CCDDZPanelNoticeSkin;
	}
	...
	private _onClickCopyKeFuWX():void{
		let _copyString = CCGlobalGameConfig.wxService;
		if(this.currentState == "delegate"){ //代理的微信号不同
			_copyString = CCGlobalGameConfig.extendWX;
		}
		CCGlobalGameConfig.copyText(this,_copyString,"微信号(" + _copyString +")");
	}
	/**
	 * 点击复制公众号微信 
	 */
	private _onClickCopyPublicWX():void{
        let _str = "好手气斗地主";
        CCGlobalGameConfig.copyText(this.parent,_str,"公众号("+_str+")");
	}
	/**
	 * 滚动到最上面
	 */
	...
	private _showRedExchange():void{
		...
		this.noticeFlag_img.source = "cc_notice_red_tit";
	}
	...
	private _showDelegate():void{
		...
		//"奖励规则：\n    1、每邀请1个好友，得1个高级红包(游戏内抽取)。\r    2、每周邀请50个好友，得50个高级红包+30元红包。\n \r微信号:" + CCGlobalGameConfig.extendWX;

		this.content_scroller.scrollPolicyV = "off";
		this._showCopyWX(true);
		
		this.noticeFlag_img.source = "cc_notice_delegate_tit";
	}
	...
	public showGameNotice():void{
		...
		this.noticeFlag_img.source = "cc_notice_game_tit";
	}
	/**
	 * 点击健康游戏忠告
	 */
	...
	private _onRemovedToStage():void{
		...
		CCDDZPanelNotice._instance = null;
	}
	/**
	 * 使能 事件
	 */
	 ...
	 * 获取公告单例
	 */
    public static getInstance(): CCDDZPanelNotice {
        if(!CCDDZPanelNotice._instance) {
            CCDDZPanelNotice._instance = new CCDDZPanelNotice();
        }
        return CCDDZPanelNotice._instance;
    }
	...
	public static remove():void{
		if(CCDDZPanelNotice._instance){
			CCDDZPanelNotice._instance.close();
		}
	}
}