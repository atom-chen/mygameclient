...
class CCDDZPanelReviveBag extends CCalien.CCDDZPanelBase {
	private static _instance: CCDDZPanelReviveBag;
	...
	init(): void {
		this.skinName = panels.CCDDZPanelReviveSkin;
	}
	...
	private _updateUI(): void {
		...
		if (this.currentState == "fullDiamondNoRecorder" || this.currentState == "fullDiamondHasRecorder") {
			...
			let sNum = CCDDZUtils.getFormatGold(this._limitNum);
			...
			this.item1Num_label.text = "x" + CCDDZUtils.getFormatGold(_info.goods[1]);
			if (_info.send) {
				this.item2Num_label.text = "x" + CCDDZUtils.getFormatGold(_info.send[0][1]);
	...
	private _onTouchClose(): void {
		this.dealAction();
		CCalien.CCDDZDispatcher.dispatch(CCGlobalEventNames.CLOSE_REVIVE_PANEL);
	}
	...
	private _onClickBuyRevive(): void {
		if (ccserver._isInDDZ) {
			ccserver.ddzDispatchEvent(1, '', { type: 5, productID: this._buyInfo.productId });
		}
		else {
			CCDDZRechargeService.instance.doRecharge(this._buyInfo.productId);
		}
	}
	...
	private _onRemovedToStage(): void {
		...
		CCDDZPanelReviveBag._instance = null;
	}
	/**
	 * 使能 事件
	 */
    ...
    */
	private _onBagRefresh(): void {
		if (this._type == 2) {
			...
			if (_nDiamond >= this._limitNum) {
				this.dealAction();
				CCalien.CCDDZDispatcher.dispatch(CCGlobalEventNames.BUY_REVIVE_SUCC);
			}
		}
	}
	...
	private _onMyInfoUpdate(): void {
		if (this._type == 1) {
			let _nGold: any = CCDDZMainLogic.instance.selfData.gold;
	...
	private _initRechargeInfo(): void {
		...
		if (this._room.roomFlag == 1) {
			let _goldRelive = CCDDZMainLogic.instance.selfData.getTodayBuyGoldReviveNum();
			if (roomId == 1001) {
				productId = 10035;
		...
		let cfg = CCGlobalGameConfig.getReviveBagInfo(productId);
	...
	public show(info): void {
		...
		e.registerOnObject(this, CCalien.CCDDZDispatcher, CCGlobalEventNames.MY_USER_INFO_UPDATE, this._onMyInfoUpdate, this);
	 ...
	 * 获取复活礼包单例
	 */
	public static getInstance(): CCDDZPanelReviveBag {
		if (!CCDDZPanelReviveBag._instance) {
			CCDDZPanelReviveBag._instance = new CCDDZPanelReviveBag();
		}
		return CCDDZPanelReviveBag._instance;
	}
	...
	public static remove(): void {
		if (CCDDZPanelReviveBag._instance) {
			CCDDZPanelReviveBag._instance.close();
		}
	}
}