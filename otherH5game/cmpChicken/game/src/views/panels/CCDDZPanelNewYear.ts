...
class CCDDZPanelNewYear extends CCalien.CCDDZPanelBase {
    private static _instance: CCDDZPanelNewYear;
	/**
	 * 规则
	 */
	 ...
	 * 新年开工礼组件
	 */
	private workComponent:CCDDZDayTaskItemInfo;
    ...
    init():void {
		this.skinName = panels.CCDDZPanelNewYear;
	}
	...
	private _formatSignRew():void{
		let _cfg = CCGlobalGameConfig.getNewYearGift();
	...
	private _enableTouch(target:eui.Image,bEnable:boolean,bGray:boolean = false):void{
		...
		if(bGray){
			target.source = "cc_common_btn_orange2_gray";
		}else{
			target.source = "cc_common_btn_orange3";
		}
	}
	...
	private _onRechargeResultNotify(event:egret.Event):void{
		...
		if(data.result == 0){
			...
			let _info = CCGlobalGameConfig.getNewYearGift();
			if(_productId == 10021){
				ccserver.isInNewyearGiftReqNewYear();
			}
			CCDDZPanelChoosePay.remove();
		}
	}
	...
	private _onClickIdxBgImg(idx:number):void{
		...
		let _state = 0;
		let _signInfo = CCDDZMainLogic.instance.selfData.getNewYearLoginInfo();
...
//1今天是否已签到（1已签到 0待签到） 2 今天是第几天 3 总签到天数 ...每天的签到情况1已签到 0 未签到
	...
	private _setGetBtnByState(state:number):void{
		let _self = this;
		let _stateLabel = {
			[0]:{text:"未领取",png:"cc_common_btn_orange3",func:function(){
				_self._enableTouch(_self.getImg,true);
			}.bind(_self)},
			[1]:{text:"已领取",png:"cc_common_btn_orange2_gray",func:function(){
				_self._enableTouch(_self.getImg,false,true);
			}.bind(_self)},
			[2]:{text:"已过期",png:"cc_common_btn_orange2_gray",func:function(){
				_self._enableTouch(_self.getImg,false,true);
			}.bind(_self)}
		}
	...
	private _setSelectSignIdx(idx:number):void{
		let _col1 = "0xB83600";
		let _png1 = "cc_newyear_bg2";
		let _col2 = "0xFFFFFF";
		let _png2 = "cc_newyear_bg3";
		let _col = _col2;
		let _png = _png2;
		this._selIdx = idx;
...
4）购买活动持续4天，签到奖励自购买当日起至第七日结束;\n\ ";
	...
	private _onClickGet():void{
		let _signInfo = CCDDZMainLogic.instance.selfData.getNewYearLoginInfo();
		if(this._buyCount < 1){
			CCDDZAlert.show("您还未购买新年礼盒,不可领取奖励!");
			return;
		}else if(this._selIdx > _signInfo[1]){
			CCDDZAlert.show("未到领取奖励时间!");
			return;
		}
		ccserver.reqGetNewYearLoginRew();
	}
	...
	private _onRemovedToStage():void{
		...
		CCDDZPanelNewYear._instance = null;
	}
	/**
	 * 使能 事件
	 */
	 ...
	 * 获取新年礼盒
	 */
    public static getInstance(): CCDDZPanelNewYear {
        if(!CCDDZPanelNewYear._instance) {
            CCDDZPanelNewYear._instance = new CCDDZPanelNewYear();
        }
        return CCDDZPanelNewYear._instance;
    }
	...
	public static remove():void{
		if(CCDDZPanelNewYear._instance){
			CCDDZPanelNewYear._instance.close();
		}
	}
}