 ...
 *
 */
class CCDDZPanelLottery extends CCalien.CCDDZPanelBase {
    private static _instance: CCDDZPanelLottery;
    public static get instance(): CCDDZPanelLottery {
        if (this._instance == undefined)
        {
            this._instance = new CCDDZPanelLottery();
        }
    ...
    protected init(): void {
        this.skinName = panels.CCDDZPanelLotterySkin;
    ...
    createChildren(): void {
        ...
        this.itemList.itemRenderer = CCDDZLotteryItem;
        ...
        ccserver.addEventListener(CCGlobalEventNames.USER_USER_INFO_RESPONSE,this.onMyUserInfoUpdate,this);
	...
	private _onAddToStage(e:egret.Event):void{
        ...
        ccserver.addEventListener(CCGlobalEventNames.USER_CoinLottery_REP,this.onLotteryStatusRep,this);
	...
	private _onRemovedToStage():void{
        ccserver.removeEventListener(CCGlobalEventNames.USER_CoinLottery_REP,this.onLotteryStatusRep,this);
        ccserver.removeEventListener(CCGlobalEventNames.USER_USER_INFO_RESPONSE,this.onMyUserInfoUpdate,this);
		...
		CCDDZPanelLottery._instance = null;
	}
	...
	private _onSelectItem(event:egret.Event):void{
        ...
        //CCDDZPanelLotteryDetail.getInstance().show(1,data);
    ...
    private _formatLotteryCfg():void{
        let _cfg:any = CCGlobalGameConfig.getCfgByField("red_coin_lottery_config");
        if(!_cfg){
            CCDDZReportor.instance.reportCodeError("red_coin_lottery_config null");
            return;
        }
        let _cfgs:any = null;
        if(!_cfg._formatData){
            ...
            CCGlobalGameConfig.setCfgByField("red_coin_lottery_config",_cfgs);
            _cfg = _cfgs;
        }
        this._allCfg = _cfg;
    }
        ...
        if(data.result == null){
            if(data.optype == 1){ //查询
                ...
                if(data.params.length >= 2 + num){
                    ...
                    CCDDZPanelLotteryDetail.updateDetail(4,this._allCfg[id]);
                }
            }
    ...
    private onMyUserInfoUpdate(event: egret.Event = null): void {
        let _curInfo : CCGlobalUserInfoData = event ? event.data: this._self;
        ...
        if(userInfoData.nickname) {
         ...
         CCDDZMainLogic.instance.selfData.redcoin = _curInfo.redcoin || 0;
        this.lbRedCoin.text = "" + CCDDZUtils.exchangeRatio(this._self.redcoin/100,true);
    }
	...
	*/
    ...
    private onMyClick(event:egret.TouchEvent):void{
        CCDDZPanelLotteryMyRec.getInstance().show();
	}
    ...
    private onRuleClick():void{
        CCDDZPanelLotteryCode.getInstance().show(2,null);
    }
}


class CCDDZLotteryItem extends eui.ItemRenderer {
    ...
    private _onClickBgImg():void{
        ...
        CCDDZPanelLotteryDetail.getInstance().show(1,this._curData);
    }
    ...
    private _initFlag():void{
        ...
        if(_data.flag == 1){
            this.flagImg.source = "cc_lotteryFlag";
    ...
    private _onWaitLuck():void{
        ...
        CCDDZPanelLotteryDetail.updateDetail(1,this._curData);
    }
    ...
    private _reqCurLuck():void{
        ...
        //console.log("_reqCurLuck---->当前时间:",CCalien.TimeUtils.timeFormatForEx(new Date(ccserver.tsServer*1000),"-","-","",true,true),"|本期结束时间:",CCalien.TimeUtils.timeFormatForEx(new Date(this.data.enddate),"-","-","",true,true));
        
        this._isReqLuck = true;
        console.log("_reqCurLuck===============>",this._id,this._phase);
        ccddzwebService.getLotteryCurLuck(this._id,this._phase,(response)=>{
    ...
    private _onGetLuck(response:any):void{
        ...
        if(response.code==1){ //流拍
            ...
            this._clearReqCurInter();
            CCDDZPanelLotteryDetail.updateDetail(2,_data);
            ...
            this._clearReqCurInter();
            CCDDZPanelLotteryDetail.updateDetail(3,_data);
        ...
        CCDDZPanelLotteryMyRec.updateOneInfo(this._curData);
    }
    ...
    protected dataChanged(): void {
       ...
       if(!this._lastUpStamp){
        ...
        CCDDZPanelLotteryDetail.updateDetail(5,_data);
        super.dataChanged();
        this._initFlag();
        let _val = _data.price.num;
        let _curStamp = ccserver.tsServer * 1000;
        let _endStamp = _data.enddate; //在格式化时已经转为毫秒
        if(_curStamp > _endStamp){
            CCDDZReportor.instance.reportCodeError("lottery startTime > endTime id:" + this._id + "|phase:" + this._phase);
        }
        _endStamp -= this._lessTime;
        
        console.log("id:" + _data.id + "|phase:" + _data.phase,"|code:",this._curStatusCode,"--dataChanged---->当前时间:",CCalien.TimeUtils.timeFormatForEx(new Date(_curStamp),"-","-","",true,true),"|本期结束时间:",CCalien.TimeUtils.timeFormatForEx(new Date(_endStamp),"-","-","",true,true));
        ...
        if(_endStamp <= _curStamp){
            ...
            if(this._curStatusCode == -1){
             ...
             _ret = CCDDZUtils.getTimeStampDiff(_curStamp,_endStamp,"all");
            if(!this._interval){
        ...
        this.moneyLabel.text = CCalien.CCDDZUtils.flatToString(_val,2);
        this.itemImg.source = CCDDZGoodsItem.parseUrl(_data.reward[0].id);
        this._tickOver = false;
        let ccddz_codeLen = _data.code.length;
        if(ccddz_codeLen > 0){
            this.hasLabel.text = "我有" + ccddz_codeLen + "个夺宝码";
