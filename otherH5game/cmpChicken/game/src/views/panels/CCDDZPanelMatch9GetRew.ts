...
class CCDDZPanelMatch9GetRew extends CCalien.CCDDZPanelBase {
    private static _instance: CCDDZPanelMatch9GetRew;
    ...
    init():void {
		this.skinName = panels.CCDDZPanelMatch9GRew;
	}
	...
	private _onRemovedToStage():void{
		...
		CCDDZPanelMatch9GetRew._instance = null;
	}
	/**
	 * 使能 事件
	 */
    ...
    private _onClickGetRew():void{
		CCDDZAlert.show('领取奖励：'+ this.get_label.text +' 成功',0,function(){
			if(!ccserver.playing){
	...
	private _initByData(roomInfo,_data):void{
		if(_data){
			...
			let reward = CCDDZMatchService.getRewardStringByRank(roomInfo, _rank, '');
			this.get_label.text = reward;

			for(let i =0;i<_result.length;++i){
				idx = i+1;
				_total = _result[i].params[0] + _result[i].params[1] + _result[i].params[2];
				_name =  CCDDZBase64.decode(_result[i].nickname);
	 ...
	 * 获取9人钻石赛结算单例
	 */
    public static getInstance(): CCDDZPanelMatch9GetRew {
        if(!CCDDZPanelMatch9GetRew._instance) {
            CCDDZPanelMatch9GetRew._instance = new CCDDZPanelMatch9GetRew();
        }
        return CCDDZPanelMatch9GetRew._instance;
    }
	...
	public static remove():void{
		if(CCDDZPanelMatch9GetRew._instance){
			CCDDZPanelMatch9GetRew._instance.close();
		}
	}
}