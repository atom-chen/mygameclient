...
class CCDDZPanelSelectAvatar extends CCalien.CCDDZPanelBase {
	private static _instance:CCDDZPanelSelectAvatar;
	public static get instance():CCDDZPanelSelectAvatar {
		if (this._instance == undefined) {
			this._instance = new CCDDZPanelSelectAvatar();
		}
	...
	protected init():void {
		this.skinName = panels.CCDDZPanelSelectAvatarSkin;
	}
	...
	createChildren():void {
		...
		this.list.itemRenderer = CCDDZIRAvatar;
		let s:number[] = [];
		for(let i = 1; i <= 8; i++){
			s.push(i);
		}
	...
	private onBtnUploadTap(event:egret.TouchEvent):void{
		/*CCalien.Native.instance.uploadImageFromDevice('选择头像', CCGlobalGameConfig.WEB_SERVICE_URL + 'upload', (args:any)=>{
	...
	show(callback:Function):void{
		...
		this.currentState = CCalien.Native.instance.isNative ? 'upload' : 'select';
	}
}

class CCDDZIRAvatar extends eui.ItemRenderer{
