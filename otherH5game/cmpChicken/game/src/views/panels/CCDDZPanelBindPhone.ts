...
class CCDDZPanelBindPhone extends CCalien.CCDDZPanelBase {
	private static _instance:CCDDZPanelBindPhone;
	public static get instance():CCDDZPanelBindPhone {
		if (this._instance == undefined) {
			this._instance = new CCDDZPanelBindPhone();
		}
	...
	protected init():void {
		this.skinName = panels.CCDDZPanelBindPhoneSkin;
	}
	...
	createChildren():void {
		...
		this.phone.prompt = ccddz_phonePrompt;
		this.phone.maxChars = ccddz_phoneLen;
		this.code.maxChars = ccddz_codeLen;
		this.code.prompt = ccddz_codePrompt;
	...
	private _onClickGetCode():void{
		...
		ccddzwebService.getCode(this.phone.text,function(response){
			let _str = {
				[0]      :  "验证码发送成功",
				[1000]   :  "参数错误",
				[2015]   :  "手机号码错误",
	...
	private _bindByExchange():void{
		...
		ccddzwebService.bindPhoneByExchange(_phone,_code,function(response){
			let _code = response.code;
			...
			let _func = function(){
				...
				CCGlobalUserData.instance.saveLocalPhone(_phone,true);
				_self.dealAction();
				CCDDZMailService.instance.getMailList(0);
				if(_okCallback){
					_okCallback();
				}
			}
			...
			CCDDZAlert3.show(_info,_func,"cc_common_btn_lab_confirm_n");
		}.bind(this))
	}
	...
	private _bindByLogin():void{
		...
		let _self = this;
		this._enableTouch(this.bindNowImg,false);
		ccddzwebService.bindPhoneByLogin(_phone,_code,this.args.unid,this.args.share_sk,function(response){
			let _code = response.code;
			let _info = response.message;
			let _phone = response.data.phone;
	...
	private _onRemoveFromStage():void{
		...
		CCDDZPanelBindPhone._instance = null;
	...
	private _initBindGold():void{
		let gold = CCGlobalGameConfig.getCfgByField("custom.bindPhoneGold");
	...
	public static nullInstance():void{
		let _ins = CCDDZPanelBindPhone._instance;
		if(_ins ){
			_ins.close(true);
		}
		CCDDZPanelBindPhone._instance = null;
	 ...
	 * 获取绑定手机号UI单例
	 */
    public static getInstance(): CCDDZPanelBindPhone {
        if(!CCDDZPanelBindPhone._instance) {
            CCDDZPanelBindPhone._instance = new CCDDZPanelBindPhone();
        }
        return CCDDZPanelBindPhone._instance;
    }
}