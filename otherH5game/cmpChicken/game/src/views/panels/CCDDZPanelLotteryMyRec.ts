...
class CCDDZPanelLotteryMyRec extends CCalien.CCDDZPanelBase {
    private static _instance: CCDDZPanelLotteryMyRec;
	...
	*/
    ...
    init():void {
		this.skinName = panels.CCDDZPanelLotteryMyRecSkin;
	...
	private _initUI():void{
		...
		this.recList.itemRenderer = CCDDZMyLottryRecItem;
	}
	...
	private _onRemovedToStage():void{
		...
		CCDDZPanelLotteryMyRec._instance = null;
	}
	...
	private _reqRecHistory(page:number =0):void{
		...
		ccddzwebService.getDoLotteryRec(_uid,page,(response)=>{
			this._isReqRecList = false;
			if(response.code == 0 ){
	 ...
	 * 获取我的夺宝记录
	 */
    public static getInstance(): CCDDZPanelLotteryMyRec {
        if(!CCDDZPanelLotteryMyRec._instance) {
            CCDDZPanelLotteryMyRec._instance = new CCDDZPanelLotteryMyRec();
        }
        return CCDDZPanelLotteryMyRec._instance;
    }
	...
	public static remove():void{
		if(CCDDZPanelLotteryMyRec._instance){
			CCDDZPanelLotteryMyRec._instance.close();
		}
	}
	...
	public static updateOneInfo(data:any):void{
		let _ins = CCDDZPanelLotteryMyRec._instance;
		if(_ins){
 ...
 * 我的夺宝记录
 */
class CCDDZMyLottryRecItem extends eui.ItemRenderer {
	/**
	 * 
	 */
	...
	private _onClickHaveCode():void{
		CCDDZPanelLotteryCode.getInstance().show(1,this.data.code_list);
	}
    ...
    protected dataChanged(): void {
		...
		let _cfgInfo = CCGlobalGameConfig.getCfgByField("red_coin_lottery_config");
