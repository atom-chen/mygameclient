...
let CCDDZ_EX_ID_GOLD = 1;
let CCDDZ_EX_ID_DIAMOND = 2;
let CCDDZ_EX_ID_WXRED = 3;
let CCDDZ_EX_ID_FEI = 4;
let CCDDZ_EX_ID_JD = 5;
let CCDDZ_EX_ID_CODE = 6;
let CCDDZ_EX_ID_HISTORY= 7;
let CCDDZ_EX_ID_AliRED = 8;

class CCDDZPanelExchange2 extends CCalien.CCDDZPanelBase {
    private static _instance: CCDDZPanelExchange2;
    public static get instance(): CCDDZPanelExchange2 {
        if (this._instance == undefined)
        {
            this._instance = new CCDDZPanelExchange2();
        }
    ...
    protected init(): void {
        this.skinName = panels.CCDDZPanelExchange2Skin;
        ...
        this.flagList.itemRenderer = CCDDZActListItem;
    ...
    private _inWXRedTime(bShowTime:boolean):boolean{
		...
		if(_custom){
			if(_custom.redExchange){
                ...
                let _str1 = CCalien.TimeUtils.timeFormatForEx(new Date(_start),"年","月","日",true);
                let _str2 = CCalien.TimeUtils.timeFormatForEx(new Date(_end),"年","月","日",true);
				this.yearTimeLabel.text = _str1 + "至" + _str2;
				_bIn = CCDDZUtils.isInTimeSection(_start,_end,_ts,true,false);
			}
		}*/
        let _info = CCGlobalGameConfig.getCfgByField("webCfg.red");
    ...
    private _inAliRedTime():boolean{
        ...
        let _bIn = false;
        let _info = CCGlobalGameConfig.getCfgByField("aliRedInfo");
    ...
    private _initFlags():void{
        let _flags:any = [
            {tit:"金豆",sel:false,new:false,act:false,id:CCDDZ_EX_ID_GOLD,cb:this._onClickGoldGroup.bind(this)},
            {tit:"钻石",sel:false,new:false,act:false,id:CCDDZ_EX_ID_DIAMOND,cb:this._onClickDiamondGroup.bind(this)}
        ]
        let _after = [
            {tit:"兑换码",sel:false,new:false,act:false,id:CCDDZ_EX_ID_CODE,cb:this._onClickCodeExchange.bind(this)},
            {tit:"兑换记录",sel:false,new:false,act:false,id:CCDDZ_EX_ID_HISTORY,cb:this._onClickExHistory.bind(this)}];

        if(this._inWXRedTime(true)){
            _flags.push({tit:"奖杯",sel:false,act:true,new:false,id:CCDDZ_EX_ID_WXRED,cb:this._onClickRed.bind(this)});
        }

        _flags.push({tit:"话费",sel:false,new:false,act:true,id:CCDDZ_EX_ID_FEI,cb:this._onClickExchangeGroup.bind(this)});

        let _cfg = CCGlobalGameConfig.exchangeConfig;
        for(let i=0;i<_cfg.length;++i){
            if(_cfg[i].goodsid ==5){
                _flags.push({tit:"京东",sel:false,act:true,new:false,id:CCDDZ_EX_ID_JD,cb:this._onClickJD.bind(this)});
                break;
            }
        }
    ...
    _filterTimeStampByGoodsId(goodsId:Array<number>):void{
        let _timeStamp = CCDDZMainLogic.instance.selfData.rcminexcexpiretime;
        if(_timeStamp){
    ...
    updateJDList(){
        ...
        this.itemList = CCGlobalGameConfig.exchangeConfig.filter((item:any)=>{
    ...
    createChildren(): void {
        ...
        this.exCodeInput.prompt = ccddz_exCodePrompt;
       ...
       // e.registerOnObject(this,this.tit_img,egret.TouchEvent.TOUCH_TAP,this.onClickTit,this);
        ...
        e.registerOnObject(this,CCalien.CCDDZStageProxy.stage,egret.Event.RESIZE,this.onResize,this);
...
//        layout.columnWidth = (this.stage.stageWidth - 40) / 3;
    ...
    private _onClickCodeExNow():void{
        ...
        if(!code || code.length< 1){
            CCDDZToast.show("兑换码不能为空");
        }else{
            let _uid = CCGlobalUserData.instance.getUid();
            ccddzwebService.doCodeExchange({uid:_uid,code:code,type: CCGlobalGameConfig.SERVER_URL_TAIL},function(response){
                CCDDZAlert.show(response.message);
            });
        }
    }
    ...
    private _showGold():void{
        ...
        let _payNum = CCDDZMainLogic.instance.selfData.getPayCount();
        //金豆界面不显示首充礼包
		for(let i=0;i<_cfg.length;++i){
    ...
    private onHistory() {
        CCDDZPanelExchange3.instance.show();
    }
    ...
    private _showWxRed():void{
        ...
        //判断是否上了红包功能
        for(let i=0;i<_l;++i){
            if(_d[i].id == CCDDZ_EX_ID_WXRED){
                this._setSelFlag(CCDDZ_EX_ID_WXRED);
                return;
            }
        }
        this._setSelFlag(CCDDZ_EX_ID_FEI);
    }
    ...
    private _showAliRed():void{
        ...
        //判断是否上了红包功能
        for(let i=0;i<_l;++i){
            if(_d[i].id == CCDDZ_EX_ID_AliRED){
                this._setSelFlag(CCDDZ_EX_ID_AliRED);
                return;
            }
        }
        this._setSelFlag(CCDDZ_EX_ID_FEI);
    ...
    show(type:number = 0,closeCb:Function = null, params: {} = null): void {
        ...
        egret.setTimeout(()=>{
            ...
            if(type == 1){
                this._setSelFlag(CCDDZ_EX_ID_DIAMOND);
            }else if(type == 2){
                this._setSelFlag(CCDDZ_EX_ID_FEI);
            }else if(type ==3){
                this._setSelFlag(CCDDZ_EX_ID_JD);
            }else if(type == 4){
                this._showWxRed();
            }else if(type ==5){
                this._setSelFlag(CCDDZ_EX_ID_HISTORY);
            }else if(type == 6) {
                this._setSelFlag(CCDDZ_EX_ID_CODE);
                this._setExCode(params);
            }
            else{
                this._setSelFlag(CCDDZ_EX_ID_GOLD);
            }
        },this,50);
    }
    ...
    private _showExHistory():void{
        let self = this;
        ccddzwebService.getExHistory(1,(response)=>{
    ...
    private _onHistoryScroll(e:egret.Event):void{
        if(this.hisoryInfoScroller.viewport.scrollV >= this.hisoryInfoScroller.height){
            ...
            if(_curPage < this._historyInfo.total_page && !_self._isReqHistory){
                ...
                ccddzwebService.getExHistory(_curPage+1,(response)=>{
    ...
    public static setHasShowClickCourse():void{
        CCalien.CCDDZlocalStorage.setItem("tipCourse","1");
    }
    ...
    private _hasShowClickCourse():boolean{
        let _show = CCalien.CCDDZlocalStorage.getItem("tipCourse") 
        if(_show != "1"){
            return false;
        }
        return true;
    ...
    close(isToShop: boolean = false): void {
        if(isToShop)
            CCalien.CCDDZPopUpManager.removePopUp(this);
        else
            super.close();       
