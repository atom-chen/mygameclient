...
class CCDDZPanelLucky extends CCalien.CCDDZPanelBase {
	private static _instance:CCDDZPanelLucky;
	...
	public static get instance():CCDDZPanelLucky {
		if (this._instance == undefined) {
			this._instance = new CCDDZPanelLucky();
		}
	...
	protected init():void {
		this.skinName = panels.CCDDZPanelLuckySkin;
	}
	...
	private _onRecvGetRewRep(e:egret.Event):void{
		...
		if(data.result == 0 ||data.result == null){
				...
				if(data.strreward){
					...
					if(this._rewInfo && this._rewInfo.length >=2){
						...
						if(this.currentState == "app"){
							CCDDZMainLogic.instance.selfData.setHasTodayAppLucky();
			...
			CCDDZAlert.show("有游戏未结束");
		}else{
			CCDDZAlert.show("请稍后重试，result:"+data.result);
		}
	}

	private _initLight():void{
		let src = "cc_lucky_light2";
		for(let i=0;i<52;++i){
			if(i%2 == 1){
				src = "cc_lucky_light1"
			}else{
    ...
    private _showGoldNotEnough(needGold): void {
        CCDDZAlert.show("金豆不足"+ needGold +"，是否前往商城购买？",1,(act)=>{
            if(act == "confirm"){
	...
	private _showNotVip():void{
		CCDDZAlert.show("您还未成为VIP用户，是否前往充值获取抽奖机会？",1,(act)=>{
			if(act == "confirm"){
				this._onTouchClose();
	...
	private _clickLucky():void{
		...
		if(this._destIdx >=0 && this._destIdx <=9){
			this["itemBgImg" + this._destIdx].source = "cc_lucky_nor";
		}

		if(isNaN(this._hasLuckNum) ){
			this._shouldStart = true;
			if(this.currentState == "lucky"){
				ccserver.reqLuckyInfo();
				return;
				...
				if(luck.hasLuck){
					CCDDZToast.show("今日抽奖已达" + lucky.dayMax +"次上限！");
					return;
				}else if(!luck.inTime){
					...
					CCDDZToast.show("免费抽奖每晚"+ beginHour + "点到" + endHour + "点开放，VIP有一次免费抽奖机会。");
					return;
				}
	...
	private _clickRule():void{
		...
		CCDDZAlert.show(str,0,null,"left");
	}
	...
	private _runNextStep(sec:number):void{
		...
		this._timeId = egret.setTimeout(()=>{
			...
			this["itemBgImg" + curIdx].source = "cc_lucky_sel";

			if(lastIdx == -1){
				lastIdx = 9;
			}
			this["itemBgImg" + lastIdx].source = "cc_lucky_nor";
	...
	private _beginLottery():void{
		...
		this._clearShowTime();
		this["itemBgImg0"].source = "cc_lucky_sel";
	...
	show(code:number):void {
		...
		let _info;
		if(code == 0){
			this._luckyCfg = CCGlobalGameConfig.getCfgByField("custom.luckyCfg");
			this.currentState = "lucky";
        	_info = CCGlobalGameConfig.getCfgByField("webCfg.luck");
			this._initCost();
		}else if(code == 1){
			this._luckyCfg = CCGlobalGameConfig.getCfgByField("custom.vipLuckyCfg");
			this._luckyCfg.hours =  this._luckyCfg.hourRange.split("|");
        	_info = CCGlobalGameConfig.getCfgByField("webCfg.vip");
			this.currentState = "vip";
		}else if(code == 2){
			this._luckyCfg = CCGlobalGameConfig.getCfgByField("custom.appLuckyCfg");
        	_info = CCGlobalGameConfig.getCfgByField("webCfg.appLucky");
			this.currentState = "app";	
		}

		this.validateNow();
		if(this.currentState == "app"){
			let noLucky = CCDDZMainLogic.instance.selfData.noTodayAppLucky();
			if(!noLucky){
