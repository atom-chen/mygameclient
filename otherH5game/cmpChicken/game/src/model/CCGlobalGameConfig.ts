...
class CCGlobalGameConfig {
	...
	static gameName: string = 'cc';       //游戏名
	static gameAlice: string = 'cc';      //游戏别名
	...
	static CARD_GAP_H: number = 80;        //卡牌横向间隔
	static CARD_GAP_V: number = 100;       //卡牌纵向间隔
	...
	static currentSortType: number = SORT_TYPE_VALUE; // 当前手牌排序的类型 0-大小   1花色
	/**
	 * 物品名称
	 */
	static _itemName = {
	 ...
	 */
	static init(): void {
		lang = new CCGlobalLanguage(RES.getRes('ccddzlang'));
		...
		//CCGlobalGameConfig.testcards = JSON.parse(egret.localStorage.getItem('testcards'));
	}

	static urlData: any;
	static getUrl(callback: Function): void {
		CCalien.Ajax.GET((this.DEBUG ? this.RESOURCE_URL : lang.config_url) + 'geturl.php', {}, (response: any) => {
			let data: any = JSON.parse(response);
			//console.log('>>>>>>>>>>' + response);

			if (!this.DEBUG) {
				CCGlobalGameConfig.WEB_SERVICE_URL = data.WEB_SERVICE_URL;//"http://192.168.0.120:8090/"
				CCGlobalGameConfig.WX_LOGIN_URL = data.WX_LOGIN_URL;
				CCGlobalGameConfig.RESOURCE_URL = data.RESOURCE_URL;

				//zhu CCGlobalGameConfig.REPORT_URL = data.REPORT_URL;
				CCGlobalGameConfig.RECHARGE_URL = data.RECHARGE_URL;
				CCGlobalGameConfig.REGISTER_URL = data.REGISTER_URL;
				CCGlobalGameConfig.PASSWORD_URL = data.PASSWORD_URL;
				CCGlobalGameConfig.HOME_PAGE_URL = data.HOME_PAGE_URL;
				CCGlobalGameConfig.DOWNLOAD_PAGE_URL = data.DOWNLOAD_PAGE_URL;
				CCGlobalGameConfig.LOBBY_PAGE_URL = data.LOBBY_PAGE_URL;
			}

			console.log('WX_LOGIN_URL:' + CCGlobalGameConfig.WX_LOGIN_URL);
			console.log('WEB_SERVICE_URL:' + CCGlobalGameConfig.WEB_SERVICE_URL);
			console.log('RESOURCE_URL:' + CCGlobalGameConfig.RESOURCE_URL);
			console.log('RECHARGE_URL:' + CCGlobalGameConfig.RECHARGE_URL);
			CCGlobalGameConfig.urlData = data;
			callback();
		}, () => {
			ccddzwebService.postError(CCGlobalErrorConfig.CONFIG_LOAD_ERROR, 'geturl.php');
		});
	}

	static getUrlData(callback: Function): void {
		callback(CCGlobalGameConfig.urlData);
	}
	...
	static initServer(ws: string): void {
		if (DEBUG || (lang.debug == "true")) {
			let localUrl: string = CCalien.CCDDZlocalStorage.getItem('server_url');
			if (localUrl) {
				ws = localUrl;
			}
		}
	...
	static init_SERVER_URL_TAIL(): void {
		let s = CCGlobalGameConfig.SERVER_URL;
		let arr = s.split('.');
		let _s1 = arr[0];
		let _b = _s1.substr(_s1.length - 1);
		CCGlobalGameConfig.SERVER_URL_TAIL = _b;
		console.log('SERVER_URL_TAIL===>', CCGlobalGameConfig.SERVER_URL_TAIL);
	}
...
let localUrl: string = CCalien.CCDDZlocalStorage.getItem('server_url');
if (localUrl) {
	CCGlobalGameConfig.SERVER_URL = localUrl;
	console.log('SERVER_URL:' + CCGlobalGameConfig.SERVER_URL);
	callback({code: 0});
} else {
	CCalien.Ajax.GET((this.DEBUG ? this.RESOURCE_URL : lang.config_url) + 'getserverurl.php', {uid}, (response: any)=> {
		...
		if (data.code == 0) {
			if (!DEBUG) {
				  CCGlobalGameConfig.SERVER_URL = data.data;
				  CCGlobalGameConfig.init_SERVER_URL_TAIL();
			}
...
// CCGlobalGameConfig.init_SERVER_URL_TAIL();
	...
	*/
	...
	public static getInviteActInfo(callback: any = null): void {
		if (1) return;
		ccddzwebService.getInviteActInfo((jInfo) => {
	...
	public static getExAliRedInfo(): void {
		ccddzwebService.getExchangeAliInfo((jInfo) => {
	...
	static loadConfigs(callback: Function): void {
		CCDDZSceneLoading.setLoadingText("加载配置。。。");
		this.allCfgs = {};
		CCalien.Ajax.GET((this.DEBUG ? this.RESOURCE_URL : lang.config_url) + 'config.php', { id: this.gameId }, (response: any) => {
			let data: any = JSON.parse(response);
			//console.log('>>>>>>>>>>' + response);
			this.allCfgs = data;
			this.dbsConfig = data.dbs_config;
			this.rid2gt = data.rid2gt;
			this.chatCost = CCGlobalGameConfig._formatChatCost(this.dbsConfig.chat_consume);
			...
			CCGlobalGameConfig._formatNewYearCfg(data.activity_config);
			...
			this.signInRewardConfig = this.dbsConfig.daysignin.map((item: string) => {
				return CCDDZUtils.parseGoodsString(item);
			});
			...
			CCGlobalGameConfig._formatFRechargeDays(this.dbsConfig.firstrechargereward.reward);
	...
	static getFRechargeInfo(): any {
		for (let i = 0; i < CCGlobalGameConfig.rechargeConfig.length; ++i) {
			if (CCGlobalGameConfig.rechargeConfig[i].firstrecharge) {
				return CCGlobalGameConfig.rechargeConfig[i];
			}
		}
		CCDDZLogUtil.error("getFRechargeInfo------error----->");
		return null;
	}
	 ...
	 * 确保在获取到支付配置后调用
	 */
	static getReviveBagInfo(product_id: number): any {
		for (let i = 0; i < CCGlobalGameConfig.rechargeConfig.length; ++i) {
			if (CCGlobalGameConfig.rechargeConfig[i].product_id == product_id) {
				return CCGlobalGameConfig.rechargeConfig[i];
			}
		}

		CCDDZLogUtil.error("getReviveBagInfo------error----->");
		return null;
	}
	...
	static isBetweenTime(cfg: any): any {
		...
		if (cfg && cfg.trigger_time && cfg.finish_time) {
			let curTimeStamp = ccserver.getServerStamp();
	...
	*/
	...
	static copyText(parent: any, str, sTip, bToast: boolean = false): void {
		...
		if (!CCalien.Native.instance.isNative) {
			...
			let _succFunc = () => {
				if (_shouldTip) {
					CCDDZToast.show("复制" + sTip + "成功");
				} else {
					CCDDZAlert.show("复制" + sTip + "成功!");
				}
			};

			let _failFunc = () => {
				CCDDZAlert.show("复制" + sTip + "失败");
			}
			...
			CCalien.Native.instance.toClipboard(copyString);
			if (_shouldTip) {
				CCDDZToast.show("复制" + sTip + "成功");
			} else {
				CCDDZAlert.show("复制" + sTip + "成功!");
		...
		if (typeof (field) == "string") {
			...
			if (cfg[field[i]]) {
				if (field.length <= 1) {
					...
					_info = CCGlobalGameConfig.getField(cfg[field[i]], field.slice(i + 1));
				}
			}
		}
		return _info;
	...
	static getCfgByField(sField: any): any {
		...
		let _info = null;
		_info = CCGlobalGameConfig.getField(this.allCfgs, _arr)
		return _info;
	}
	...
	static toUrl(url: string): void {
		...
		if (cc_nativeBridge.isNative) {
			cc_nativeBridge.openUrlBySys(url)
		} else {
	...
	static showKeFu(): void {
		let _str = CCGlobalGameConfig.wxService
		let _ins = CCDDZPanelAlert3.instance;
		...
		_ins.show(_info, btnNum, function (act) {
			if (act == "confirm") {
				if (_ins.btnConfirm.labelIcon == "cc_room_report") {
					CCDDZReportor.instance.toReportPage();
				} else {
					CCGlobalGameConfig.copyText(this.parent, _str, "微信号(" + _str + ")");
				}
			} else if (act == "cancel") {
				CCDDZReportor.instance.toReportPage();
			}
		...
		if (btnNum == 1) {
			_ins.btnConfirm.labelIcon = "cc_common_copyWX";
		} else {
			_ins.btnConfirm.labelIcon = "cc_room_report";
		}
		_ins.btnCancel.labelIcon = "cc_room_report";
	}
	...
	static getMySelfUrl(): string {
		...
		let _url = CCGlobalGameConfig.WX_LOGIN_URL + "?state=3&" + "share_sk=" + sk;
	...
	static initGameUrlsCfg(callback: Function): void {
		ccddzwebService.getGameUrlsCfg((cfgs) => {
