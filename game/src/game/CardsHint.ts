...
class CardsHint {
	...
	static getCardsHash(cards:Array<number>):{}{
		...
		for(var i = 0; i < cards.length; ++i){
			key = CardsHint.getCardLogicValue(cards[i]);
			if(!hash[key]){
				hash[key] = [cards[i]];
			}else{
			...
			if(hasTab[k].length == 4){
				if(max_amount < 8){
					...
					if(combs && combs.length == 4){
						...
						ans = {node:CardsType.GetType(tc), cards:tc};
				...
				if(max_amount < 4){
					ans = {node:CardsType.GetType(hasTab[k]), cards:hasTab[k]};
					max_amount = 4;
				}
			}
		}
			...
			for(var j = i + 1; j < 13; ++j){
				if(hasTab[j]){
					...
					if(j == 12 && line.length > 4 && line.length > max_amount){
						ans = {node:CardsType.GetType(line), cards:line};
	...
	static getMaxAmountCards(cards:Array<number>):any{
		...
		var temp = CardsType.GetType(cards)
		if(temp.ct > 0){
			return cards;
		}
		
		temp = CardsHint.getAllCardsPossible(cards);
...
// let SANSHUN:number = 12;
	 ...
	 static getFitCards(cards:Array<number>, idx:number, precards:Array<number> = []):any{
		...
		if(precards.length > 0){
			prenode = CardsType.GetType(precards);
