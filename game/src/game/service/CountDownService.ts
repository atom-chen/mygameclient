...
class CountDownService extends Service{
	...
	public static get instance():CountDownService {
		if (this._instance == undefined) {
			this._instance = new CountDownService();
		}
	...
	private onTimer():void{
		...
		this._items.forEach((item:any, index:number)=> {
			let time:number = item.end - server.tsServer;
	...
	static register(second:number, onSecond:Function, onComplete:Function = null):number{
		return CountDownService.instance.register(second, onSecond, onComplete);
	}

	static unregister(id:number):void{
		CountDownService.instance.unregister(id);
	}
}