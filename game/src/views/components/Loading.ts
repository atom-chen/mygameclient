...
class Loading extends eui.Group implements alien.ITicker{
	...
	update():void {
		alien.Utils.enumChildren(this._container, (item:LoadingItem)=>{
			item.update();
		});
	}
}
...
class LoadingItem extends egret.Bitmap{
	...
	constructor(){
		super(RES.getRes(ResNames.loading_icon));
