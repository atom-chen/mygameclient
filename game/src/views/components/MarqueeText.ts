 ...
 *
 */
class MarqueeText extends eui.Component {
    public constructor() {
        super();
    }
...
//        Dispatcher.addListener(EventNames.CLEAR_LABA,this.clear,this);
        this.skinName = components.MarqueeSkin;
//        this.txt.mask = this.maskR;
    ...
    protected addToStage(e: egret.Event): void {       
        this.x = alien.StageProxy.stage.stageWidth - this.width >> 1;
        this.y=114;
    ...
    protected removeFromStage(e: egret.Event): void {
        alien.StageProxy.stage.removeEventListener(egret.Event.RESIZE,this.onResize,this);
        this.clear();
    }
...
//        this.addEventListener(egret.TouchEvent.TOUCH_ROLL_OUT,this.onMouseOut,this);
    ...
    private deleteAnnounce():void{
        ExchangeService.instance.deleteAnnounce();
    ...
    private _showOneMsgByChouRec(data:any):void{
        ...
        _ret.text= "恭喜<font color='#F0FF00'>" + data.nickname.substr(0,9)+ "</font>在"+ name +"抽得<font color='#F0FF00'>" + Utils.exchangeRatio(data.money,true) + "</font>" + " 奖杯";
    ...
    public showLast(type:number = 1):void{
        ...
        if(_exIns.announce && _exIns.announce.length > 0){
            ...
            let _data = MainLogic.instance.selfData.getHornTalkRecList(); //用户喇叭消息
            if(_data.length > 0){
            ...
            let _data = ExchangeService.instance.getOneChouMaxInfo();
            if(_data){
    ...
    private getNextText(): any {
        ...
        if(_exIns.announce && _exIns.announce.length > 0){
            ...
            if(data){
                ...
                str += "恭喜<font color='#F0FF00'>" + data.nickname.substr(0,9)+ "</font>在"+ name +"抽得<font color='#F0FF00'>" + Utils.exchangeRatio(data.money,true) + "</font>" + "奖杯";
                _ret.text = str;
            }else{
                _ret.text = "";
            }
    ...
    public _onClickOpenHornTalk():void {
        PanelHornTalk.getInstance().show();
    }
    ...
    private _getRankNext():void{
       let _infos = this["inviteRankList"] || [];
        ...
        if(_one){
            let _ret ={type:4,text:"",time:3000}; //Utils.exchangeRatio(_one.coin / 100,true) 
