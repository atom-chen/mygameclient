...
class ShopItem extends eui.ItemRenderer {
    ...
    private _refreshLeftTime():void {
        let _cur = alien.Utils.getCurTimeStamp();
        let _minute = alien.Utils.getTimeStampDiff(_cur,this.data.timeStamp,"minute");
        ...
        if(_hour == 0 && _minute == 0){
            ...
            PanelExchange2.instance.onTimeExpired([this.data.goodsid]);
            return;
        }
    ...
    private _doExchange(){
        var tmp:string;
        if(PanelExchange2.instance.lv_goods.selectedItem)
        {
            let _info = PanelExchange2.instance.lv_goods.selectedItem;
            
            let _self = this;
            if(_info.product_id!=0){ //兑换非游戏内道具需要绑定手机号
                let _bBindPhone = UserData.instance.hasBindPhone();
                if(!_bBindPhone){
                    PanelBindPhone.getInstance().show(2,{showClose:true},function(){
                        _self._doExchange();
                    });
                    return;
                }
            ...
            if(_info.goodsid==0)
            {
                tmp = alien.StringUtils.format(lang.exchange_redcoin_gold,amount);
            }
            else if(_info.goodsid == 1) {
                tmp = alien.StringUtils.format(lang.exchange_redcoin_red,amount);
                /*if(1){
                    Alert.show("由于微信冻结公众号商户，导致奖励无法兑换，官方正在增加新的奖励通道。感谢您对好手气斗地主一贯的支持！");
                    return;
                }*/
            } 
            else if(_info.goodsid == 4){ //话费
                /*if(_info.amount==1 || _info.amount==5){
                    Alert.show("因为运营商年底维护，5元暂时停止兑换，预计元旦左右恢复正常！",0,null);
                    return;
                }*/
                let sInfo = "确定兑换{0}元话费？";
                tmp = alien.StringUtils.format(sInfo,_info.amount);
            }else if(_info.goodsid == 6){ //话费卡
                let sInfo = "确定兑换{0}元话费充值卡？\n兑换成功后不可退换\n\r";
                tmp = alien.StringUtils.format(sInfo,_info.amount);
                _textFlow = (new egret.HtmlTextParser).parser(tmp + "\n<font color='#FF0000'>" + this._buildFeiCardSupport(_info) + "</font>");
                ...
                tmp = alien.StringUtils.format(sInfo,_info.num);
            }
        
            Alert.show(tmp,1,(action: string) => { 
                if(action == 'confirm') {
                    let _needNum = 0;
                    if(_info.money > 0 || _info.coin > 0){ //奖券兑换
                        ...
                        let _myRedNum = (MainLogic.instance.selfData.redcoin || 0) /100 ;
                        ...
                        let _myRedNum = BagService.instance.getItemCountById(3);
                        if(_myRedNum < _needNum){
                            egret.setTimeout(()=>{
                                Alert.show("钻石不足！");
                            },this,300);
                ...
                ExchangeService.instance.doRedExchange(_info);
                }         
            },"center",_textFlow);   
        }
    }
     ...
     */
    private _doBuy(){
        RechargeService.instance.doRecharge(this.data.product_id);
    }
    ...
    protected dataChanged(): void {
        super.dataChanged();
        let _bShow = false;
        //this.imgIcon.source = GoodsItem.parseUrl(this.data.goodsid);
        ...
        if(this.data.goodsid == 4){
            ...
            _descInfo = Utils.exchangeRatio(this.data.amount,true);
        }
        else{
        ...
        if(this.data.money && this.data.money > 0){
            this.exchange_img.source = "icon_redcoin";
            ...
            this.exchange_img.source = "icon_diamond";
        }

        this.labGold.text = _descInfo;

		this.imgIcon.source = GoodsItem.parseUrl(_iconId);
		//this.labScale.text =  _desc +  Utils.currencyRatio(this.data.cost_scale) + _end;
        ...
        this.btnConfirm.skin._Image1.source = "common_btn_orange2_n";
        ...
        if(this.data.amount){ //兑换
            ...
            if(this.data.goodsid==0)
                ...
                this.labGold.text = _pre + Utils.exchangeRatio(this.data.amount,true); 
