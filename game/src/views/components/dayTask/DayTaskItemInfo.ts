 ...
 */
class DayTaskItemInfo extends eui.Component {
    /**
     * 任务描述
     */
     ...
     * 做任务 按钮
     */
    private mDoTask_btn:ButtonDoTaskSkin;
	...
	private _onAddToStage():void{
        server.addEventListener(EventNames.USER_GET_TASKREW_REP,this._onRecvGetRewRep,this);
	}
    /**
     * 移除舞台
     */
    ...
    private _initRew0ByIdAndNum(id:number,num:number){
        if(id == 41003){ //高级红包次数
            ...
            this.mReward0_img.source = "room_0";
    ...
    private _initRew1ByIdAndNum(id:number,num:number){
        if(id == 1){ //红包余额
            ...
            this.mReward1_img.source = "room_2";
            this.mReward1Num_label.text = num +"小时";
        }else if(id == 0){
            this.mReward1_img.source = "room_0";
            this.mReward1Num_label.text = "" +num;
        }else if(id == 3){
            this.mReward1_img.source = "room_3";
            this.mReward1Num_label.text = "" +num;
        }
    }
    ...
    private _onPlayNormalMatch(roomIds:any):void{
        PanelDayTask.getInstance().close();
        ...
        alien.SceneManager.instance.show(SceneNames.ROOM,args);
    }

    /**
     * 任务是比赛
     */
    private _onPlayMatch(): void {
        PanelDayTask.getInstance().close();
        alien.PopUpManager.instance.removeAllPupUp();
    ...
    private _onGetRewardReq():void{
        ...
        if(this._data.tp ==2){ //阶段性任务
            data.optype = 2;
        } else if(this._data.tp == 98) {//捕鱼任务
            webService.goGetTaskReward(server.uid, this._data.id);
            return ;
        }
        else if(this._data.tp == 1 && this._data.id == 6){ //新年任务
		    server.reqGetNewYearTaskRew(this._data.id,this._data.subId,this._data.type);
            return;
	    }

        server.send(EventNames.USER_GET_TASKREW_REQ,data );
    }
    ...
    private _onRecvGetRewRep(e: egret.Event):void{
        let data = e.data;
        if(!this._data || data.taskid != this._data.id) return;
        ...
        Alert.show(_str,0,null,_align);
        let _type = this._data.tp ;
        let myData = MainLogic.instance.selfData;
        //奖励领取成功，按钮状态设置为已领取
        if(e.data.result == 0){
            if(_type ==1 || _type == 3|| _type == 4||_type == 5||_type == 6||_type == 7||_type ==99){
    ...
    private _initBtnAndFunc(status:number):void{
        if(this._tStatusFunc[status]){
            ...
            LogUtil.info("_initBtnAndFunc=====error=======>",status)
        }
    }
    ...
    private _onNotOverBtn():void{
        ...
        if(this._data.tp == 1){ //赢多少局 斗地主
           ...
           this.mDoTask_btn.setClickFunc(this._onPlayNormalMatch.bind(this,this._data.roomid));
        }else if(this._data.tp == 6){ //比赛
           this.mDoTask_btn.setClickFunc(this._onPlayMatch.bind(this));
        }else if(this._data.tp == 7){ //付费比赛
           this.mDoTask_btn.setClickFunc(this._onPlayMatch.bind(this));
        }else if(this._data.tp == 98){//捕鱼            
           this.mDoTask_btn.setClickFunc(this._onPlayFish.bind(this));
        }
    ...
    private _onWebGetTaskRewardSuc(evt: egret.Event): void {
        ...
        if(!this._data || data.taskid != this._data.id) return;
        let _self = this;
        if(data.taskid == _self._data.id) {
            this._onHasGetRewardBtn();
        }
    }
}

window["DayTaskItemInfo"]=DayTaskItemInfo;