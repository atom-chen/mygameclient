...
class GoldRainEffectLayer extends eui.Group {
	...
	play():void{
		if(!this._goldItems){
			this._goldItems = [];
			let factory:egret.MovieClipDataFactory = new egret.MovieClipDataFactory(RES.getRes(ResNames.ani_coin_json), RES.getRes(ResNames.ani_coin_png));
			for(let i = 0; i < 35; i++){
		...
		alien.SoundManager.instance.playEffect(ResNames.effect_gold_rain);
	}
}
...
window["GoldRainEffectLayer"]=GoldRainEffectLayer;