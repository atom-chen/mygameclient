 ...
 * 每日任务信息面板
 */
let TASK_TYPEID_DIAMOND = 1;
let TASK_TYPEID_DAY = 2;
let TASK_TYPEID_YEAR = 3;

class PanelDayTask extends alien.PanelBase {
    private static _instance: PanelDayTask;
    ...
    init():void {
		this.skinName = panels.PanelDayTaskSkin;
	...
	private _formatEveryTaskData():any{
		let _task = GameConfig.getDayTaskList();
		if(!_task) return [];
		...
		for(let i=0;i<_task.length;++i){
			...
			if(_task[i].tp == 1){
				...
				}else if(_task[i]["roomid"]["1000"]||_task[i]["roomid"]["1006"]||_task[i]["roomid"]["8003"] || _task[i]["roomid"]["8000"]){
			...
			}else if(_task[i].tp == 6){ //比赛
				if(_task[i].id == 16){
					_oneItemInfo.taskDesc1 = "参加任意1场比赛";
					_oneItemInfo.taskDesc2 = "";
					_oneItemInfo.num = _taskPro[6]||0;
					_oneItemInfo.total = 1;
				}else if(_task[i].id == 17){
					_oneItemInfo.taskDesc1 = "参加任意5场比赛";
					_oneItemInfo.taskDesc2 = "";
					_oneItemInfo.num = _taskPro[6]||0;
					_oneItemInfo.total = 5;
				}
				if(this._rewHasDiamond(_idAndNumArr)){
					_oneItemInfo.diamond = true;
				}
			}else if(_task[i].tp == 7){ // 比赛
				if(_task[i].id == 18){
					_oneItemInfo.taskDesc1 = "参加任意1场100奖杯付费赛";
					_oneItemInfo.taskDesc2 = "";
					_oneItemInfo.num = _taskPro[7]||0;
					_oneItemInfo.total = 1;
				}
				if(this._rewHasDiamond(_idAndNumArr)){
					_oneItemInfo.diamond = true;
			...
			if(_task[i].day == 1){
				//目前仅保留赢多少局和玩多少局任务
				if(_task[i].tp == 1 || _task[i].tp == 3 || _task[i].tp == 4 || _task[i].tp == 5 || _task[i].tp == 6 || _task[i].tp == 7 || _task[i].tp == 99){
		...
		for(let i = 0; i < GameConfig.fishTask.length; i++) {
			...
			_oneItemInfo.id = _task.taskId;
			_oneItemInfo.status = _task.status || 0;
			_oneItemInfo.tp = 98;
			_oneItemInfo.total = _task.target;
			_oneItemInfo.taskDesc1 = _task.title;
			_oneItemInfo.taskDesc2 = "";
			_oneItemInfo.num = _task.over;

			_idAndNumArr =[];
			_items = _task.reward.split("|");
			for(let j=0;j<_items.length;++j){
				_idAndNum = _items[j].split(":");
				_itemId = Number(_idAndNum[0]);
				_itemNum = Number(_idAndNum[1]);
	...
	private _initLeftBtns():void{
		this._leftLabels = [
			{tit:"钻石任务",sel:false,new:false,act:false,id:TASK_TYPEID_DIAMOND,cb:this._onClickDiamondTask.bind(this)},
			{tit:"金豆任务",sel:false,new:false,act:false,id:TASK_TYPEID_DAY,cb:this._onClickDayTask.bind(this)},
		]
	...
	private _initYearTask():void{
		...
		this.yearList.itemRenderer = IRContainer;
		this.yearList.dataProvider = this._yearProvider;
		
		this._initLeftBtns();

		let _isInNewYearTask =GameConfig.isInNewYearTask();
		if(_isInNewYearTask.isInTime){
			let _hasClick= UserData.instance.hasClickNewYearTask();
			this._leftLabels.push({tit:"国庆福利",sel:false,new:!_hasClick,act:false,id:TASK_TYPEID_YEAR,cb:this._onClick51Task.bind(this)})
			server.isInNewyearTaskReqNewYear();
			let _cfg = GameConfig.newYearCfg.yearTask;
	...
	private _initDefaut():void{
	    ...
	    this.mTask_list.itemRenderer = IRContainer;
    ...
    private _onDayTaskRewGetSucc():void{
		...
		else if(_select.id == TASK_TYPEID_DAY){
			this._showDayTask();
		}else if(_select.id == TASK_TYPEID_DIAMOND){
	...
	private _toTaskTop():void{
		this.taskScroller.stopAnimation();
		this.taskScroller.viewport.scrollV = 0;
		this.taskScroller.validateProperties();
	}
	 ...
	 * 
	 */
	private _showYearGrp(bShow:boolean):void{
		this["yearGrp"].visible = bShow;
	}
	...
	private _showYearTask():void{
		...
		this._showYearGrp(true);
		this.yearScroller.stopAnimation();
		this.yearScroller.viewport.scrollV = 0;
		this.yearScroller.viewport.validateProperties();
		...
		if(!_shunInfo){
			_shunInfo = {progress:0,status:[0,0,0]}
		}

		if(!_planeInfo){
			_planeInfo = {progress:0,status:[0,0,0]}
		}

		let _cfg = GameConfig.getNewYearTask();
	...
	private _onClick51Task():void{
		//console.log("===========_onClickDayTask=====>");
		UserData.instance.setHasClickNewYearTask();
		this._showYearTask();
	}
	...
	private _onAddToStage():void{
		...
		e.registerOnObject(this,server,EventNames.USER_OPERATE_REP,this._onRecvUserOperateRep,this);
	...
	private _addListener(bEnable:boolean):void{
		...
		alien.Dispatcher[_func](EventNames.WEB_GET_TASK_REWARD_SUC,this._onWebGetTaskRewardSuc,this);
	}
	...
	private _userinfoUpdate():void{
		...
		if(item.id == TASK_TYPEID_DAY){
			this._showDayTask();
		}else if(item.id == TASK_TYPEID_DIAMOND){

		}else if(item.id == TASK_TYPEID_YEAR){
			this._showYearTask();
		}
	}
...
/**
    ...
    private _onRecvUserOperateRep(event:egret.Event):void{
		...
		if(data.optype == 4){ //查询活动
			...
			if(data.result == null && data.params && data.params.length == 3){
				...
				Alert.show("领取奖励失败|result:" + data.result);
			}
		}
    }
	...
	public show():void{
		...
		this._setLeftSelectById(TASK_TYPEID_DIAMOND);
	}
	
	/**
	 * 获取每日任务单例
	 */
    public static getInstance(): PanelDayTask {
        if(!PanelDayTask._instance) {
            PanelDayTask._instance = new PanelDayTask();
        }
        return PanelDayTask._instance;
    }
	...
	public static remove():void{
		if(PanelDayTask._instance){
			PanelDayTask._instance.close();
		}
	}

	private _onWebGetTaskRewardSuc(evt: egret.Event): void {
		this._onDayTaskRewGetSucc();
		 MainLogic.instance.selfData.setTaskHasGetRew(-1);
	}
}