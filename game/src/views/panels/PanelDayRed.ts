...
class PanelDayRed extends alien.PanelBase {
	private static _instance:PanelDayRed;

	private _okList:any;
	private _notOkList:any;
	public static get instance():PanelDayRed {
		if (this._instance == undefined) {
			this._instance = new PanelDayRed();
		}
	...
	protected init():void {
		this.skinName = panels.PanelInviteDayRed;
		this._okList = [];
		this._notOkList = [];
	}
	...
	private _addListener(bAdd:boolean):void{
		...
		InviteService.instance[func](EventNames.INVITE_DATA_REFRESH, this.onInviteDataUpdate,this)
	...
	private _showGetRedByResult(data:any):void{
		...
		if(data.result == 0 ||data.result == null){
			...
			if(data.params && data.params.length >= 1){ 
				let _sMoney = Utils.exchangeRatio(data.params[0] / 100,true);
				Alert.show("恭喜获得" + _sMoney + "奖杯！");
			}
			this._updateUI();
		}
	...
	private _updateUI():void{
		...
		for(let i=0;i<5;++i){
			...
			if(i < hasN + hasGetN){
				...
				if(i < hasGetN){
					this["friendGrp" + i].visible = false;
					this["redBgImg" + i].source = "invite_day_red";
					this["inviteImg" + i].source = "invite_day_hasget";
					this["inviteImg" + i].touchEnabled = false;
					this["redBgImg" + i].touchEnabled = false;
				}else{
					this["friendGrp" + i].visible = false;
					this["redBgImg" + i].source = "invite_day_red";
					this["inviteImg" + i].source = "invite_day_get";
				}
			}else{
				this["inviteImg" + i].source = "invite_day_get1";
				if(i < notOkLen){
					this["nameLabel" + i].text = this._notOkList[i].nickname.substr(0,6);
					this["headIcon" + i].source = this._notOkList[i].headimgurl2;
					this["redBgImg" + i].source = "invite_day_bg";
					...
					this["inviteImg" + i].visible = true;
					this["friendGrp" + i].visible = false;
					this["redBgImg" + i].source = "invite_day_red1";
					this["statusLabel" + i].visible = false;
				}
			}
		}
	...
	private _onClickGetRed(idx):void{
		if(this["inviteImg" + idx].source == "invite_day_get"){
			let hasN = MainLogic.instance.selfData.getInviteRedNum();
			let hasG = MainLogic.instance.selfData.getInviteRedGetRewNum();
			let maxN = GameConfig.getCfgByField("custom").inviteRedNum;
			if(hasN >0){
				if(hasG < maxN){
