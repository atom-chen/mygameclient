...
class PanelLotteryDetail extends alien.PanelBase {
    private static _instance: PanelLotteryDetail;
	...
	*/
    ...
    init():void {
		this.skinName = panels.PanelLotteryDetail;
	...
	private _initUI():void{
		...
		this.luckList.itemRenderer = LuckItem;
	...
	private _calculateRightNum():string{
		let _redCoin = MainLogic.instance.selfData.redcoin / GameConfig.exchangeRatio / 100;
		
		let _price = this._detailData.price.num ||1;
		let _num = _redCoin / _price;
		let _num1 = alien.Utils.flatToString(_num,0);
		return _num1;
	}
	...
	private _isSingleMaxNum(num:number):boolean{
		if(num > GameConfig.lotteryMaxNum){
			return true;
		}
		return false;
	}
	...
	private _showMaxSingleInfo():void{
		Toast.show("单次最大投入"+GameConfig.lotteryMaxNum);
	}
	...
	private _onClickHaveCode():void{
		PanelLotteryCode.getInstance().show(1,this._detailData.code);
	}
	...
	private _isEnoughRedCoin(needMoney:number):boolean{
		...
		let _myRed = _data.redcoin / GameConfig.exchangeRatio / 100;
		if(_myRed >= needMoney){
			return true;
		}
		return false;
	...
	private _onClickJoinLottery():void{
		...
		if(_isEnough){
			...
			server.reqBuyLotteryById(_id,_num);
		}else{
			Alert.show("奖杯不足,前往游戏赢取奖杯",0,function(act){
				if(act == "confirm"){
        			alien.SceneManager.instance.show(SceneNames.ROOM,{toGold:true});
				}
			});
		}
	}
	...
	private _initTotMoney():void{
		...
		if(this._isSingleMaxNum(_nCount)){
			this._showMaxSingleInfo();
			_nCount = GameConfig.lotteryMaxNum;
		}
		this.moneyLabel.text = "" + alien.Utils.flatToString(this._detailData.price.num *_nCount,2);
	}
	...
	private _enableEvent(bEnable:boolean):void{
		...
		if(!bEnable){
		 ...
		 alien.Dispatcher[_func](EventNames.MY_USER_INFO_UPDATE,this._onMyInfoChange,this);
	...
	private _onRemovedToStage():void{
		...
		PanelLotteryDetail._instance = null;
	}
	...
	public showDetail(_data:any):void{
		...
		this.timeLabel.text = alien.TimeUtils.timeFormatForEx(new Date(_data.enddate),"-","-","",true) + "开奖"
	...
	private _reqLuckHistory(page:number =0):void{
		...
		webService.getLotteryLuckHistory(page,this._detailData.id,(response)=>{
			this._isReqLuckList = false;
			if(response.code == 0 ){
	 ...
	 * 获取夺宝详情
	 */
    public static getInstance(): PanelLotteryDetail {
        if(!PanelLotteryDetail._instance) {
            PanelLotteryDetail._instance = new PanelLotteryDetail();
        }
        return PanelLotteryDetail._instance;
    }
	...
	public static remove():void{
		if(PanelLotteryDetail._instance){
			PanelLotteryDetail._instance.close();
		}
	}
	...
	public static updateDetail(type:number,data:any):void{
		let _ins =  PanelLotteryDetail._instance;
		if(_ins){
 ...
 * 往期幸运儿
 */
class LuckItem extends eui.ItemRenderer {
	/**
	 * 开奖时间
	 */
	private timeLabel:eui.Label;
