...
class PanelFirstRecharge extends alien.PanelBase {
    private static _instance: PanelFirstRecharge;
	/**
	 * 正常的字
	 */
	 ...
	 * 去充值按钮
	 */
	private mGoRecharge_btn:ButtonDoTaskSkin;
	 ...
	 */
	init():void{
		this.skinName = panels.PanelFirstRecharge;
	}
	...
	private _onGoRecharge():void{
		let _info = GameConfig.getFRechargeInfo();
		if(!_info) return;

		RechargeService.instance.doRecharge(_info.product_id);
	}
	...
	private _playLightOnTarget(target:eui.Image):any{
		let mcData = RES.getRes("frLightAni_json");
		let pngData = RES.getRes("frLightAni_png");
	...
	private _onRemoveFromStage():void{
		...
		PanelFirstRecharge._instance = null;
	 ...
	 * 获取首充UI单例
	 */
    public static getInstance(): PanelFirstRecharge {
        if(!PanelFirstRecharge._instance) {
            PanelFirstRecharge._instance = new PanelFirstRecharge();
        }
        return PanelFirstRecharge._instance;
    }
	...
	public static setDisableGet():void{
		if(PanelFirstRecharge._instance) {
			PanelFirstRecharge._instance.mGoRecharge_btn.visible = false;
		}
	}
	...
	public static remove():void{
		if(PanelFirstRecharge._instance){
			PanelFirstRecharge._instance.close();
		}
	}
