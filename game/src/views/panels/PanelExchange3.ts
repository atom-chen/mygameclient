...
class PanelExchange3 extends alien.PanelBase {
    private static _instance: PanelExchange3;
    public static get instance(): PanelExchange3 {
        if (this._instance == undefined)
        {
            this._instance = new PanelExchange3();
        }
    ...
    protected init(): void {
        this.skinName = panels.PanelExchange3Skin;
    ...
    createChildren(): void {
        ...
        this.lv_list1.itemRenderer = IRExchangeHistory;
        this.lv_list2.itemRenderer = IRExchangeDetail;
    ...
    private loadConfigs(callback: Function): void {
        webService.getRedcoinHistory(this._self.uid,(response: any) => {
...
class IRExchangeHistory extends eui.ItemRenderer {
    ...
    protected dataChanged(): void {
        ...
        if (this.data.recordtime){ 
            ...
            let format = alien.TimeUtils.timeFormatForEx(dateTime);
        ...
        if (this.data.type == "1"){ 
            ...
            this.lbl_change.text = "+" + Utils.exchangeRatio(this.data.coin/100,true);
            this.lbl_change.textColor = 0xFE0100;
        } else if(this.data.type == "2"){
            this.lbl_dis.text = "兑换" + this.data.goodsamount + GameConfig.exchangeGoodsConfig[this.data.goodsid].describe
            this.lbl_change.text = "-" + Utils.exchangeRatio(this.data.coin/100,true);
            this.lbl_change.textColor = 0x4e7904;
        }else if (this.data.type == "3"){ 
            let _name = "";            
            if(!lang.exchange_roomid[this.data.roomid]) {
                _name = lang.match_reward;
            }
            else {
                _name = lang.exchange_roomid[this.data.roomid] + "奖杯";
            }
            this.lbl_dis.text = _name;//[this.data.roomid];
            this.lbl_change.text = "+" + Utils.exchangeRatio(this.data.coin/100,true);
            this.lbl_change.textColor = 0xFE0100;
...
class IRExchangeDetail extends eui.ItemRenderer {
    ...
    protected dataChanged(): void {
        ...
        {
            this.lbl_dis.text = "兑换" + this.data.goodsamount + GameConfig.exchangeGoodsConfig[this.data.goodsid].describe
//            this.lbl_dis.text = GameConfig.exchangeGoodsConfig[this.data.goodsid].describe;
        }
        if (this.data.recordtime)
        {
            let dateTime = new Date(this.data.recordtime * 1000);
