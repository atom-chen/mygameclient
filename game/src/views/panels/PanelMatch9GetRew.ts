...
class PanelMatch9GetRew extends alien.PanelBase {
    private static _instance: PanelMatch9GetRew;
    ...
    init():void {
		this.skinName = panels.PanelMatch9GRew;
	}
	...
	private _onRemovedToStage():void{
		...
		PanelMatch9GetRew._instance = null;
	}
	/**
	 * 使能 事件
	 */
    ...
    private _onClickGetRew():void{
		Alert.show('领取奖励：'+ this.get_label.text +' 成功',0,function(){
			if(!server.playing){
	...
	private _initByData(roomInfo,_data):void{
		if(_data){
			...
			let reward = MatchService.getRewardStringByRank(roomInfo, _rank, '');
			this.get_label.text = reward;

			for(let i =0;i<_result.length;++i){
				idx = i+1;
				_total = _result[i].params[0] + _result[i].params[1] + _result[i].params[2];
				_name =  Base64.decode(_result[i].nickname);
	 ...
	 * 获取9人钻石赛结算单例
	 */
    public static getInstance(): PanelMatch9GetRew {
        if(!PanelMatch9GetRew._instance) {
            PanelMatch9GetRew._instance = new PanelMatch9GetRew();
        }
        return PanelMatch9GetRew._instance;
    }
	...
	public static remove():void{
		if(PanelMatch9GetRew._instance){
			PanelMatch9GetRew._instance.close();
		}
	}
}