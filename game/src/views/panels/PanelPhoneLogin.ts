 ...
 * 关注公众号活动面板
 */
class PanelPhoneLogin extends alien.PanelBase {
    private static _instance: PanelPhoneLogin;
    ...
    init():void {
		this.skinName = panels.PanelPhoneLoginSkin;
	}
	...
	createChildren():void {
		...
		this.phoneLabel.prompt = _phonePrompt;
		this.phoneLabel.maxChars = _phoneLen;
		this.codeLabel.maxChars = _codeLen;
		this.codeLabel.prompt = _codePrompt;
	...
	private _onClickGetCode():void{
		...
		webService.getCode(this.phoneLabel.text,function(response){
	...
	private _onClickLogin():void{
		...
		webService.loginByPhone(_phone,_code,_self.unid,_self.sk,function(response){
			_self.dealAction();
			LoginService.instance.onVerifyResponse(response);
		}.bind(_self))
	}
	...
	private _onRemovedToStage():void{
		...
		PanelPhoneLogin._instance = null;
	}
	/**
	 * 使能 事件
	 */
	 ...
	 * 获取手机登录单例
	 */
    public static getInstance(): PanelPhoneLogin {
        if(!PanelPhoneLogin._instance) {
            PanelPhoneLogin._instance = new PanelPhoneLogin();
        }
        return PanelPhoneLogin._instance;
    }
	...
	public static remove():void{
		if(PanelPhoneLogin._instance){
			PanelPhoneLogin._instance.close();
		}
	}
}