...
class PanelFeiEx extends alien.PanelBase {
	private static _instance:PanelFeiEx;
	public static get instance():PanelFeiEx {
		if (this._instance == undefined) {
			this._instance = new PanelFeiEx();
		}
	...
	protected init():void {
		this.skinName = panels.PanelFeiExSkin;
		this._exchangeFunc = null;
		this._okFunc = null;
		this._feiKaCfg = {
			cmcc:{func:this._onSelectYD.bind(this)},
			cucc:{func:this._onSelectLT.bind(this)},
			ctcc:{func:this._onSelectDX.bind(this)},
		};
	}
     ...
     * 话费卡支持的运营商
     */
    private _buildFeiCardSupport(info):string{
        let support = info.support;
        let sPre = info.amount +"元充值卡";
        let str = "目前仅支持"
        if (support.cmcc && support.cucc && support.ctcc){
            str = "目前支持移动、联通和电信";
        }else{
            let before = false;
            if (support.cmcc){
	...
	private _onAlertFei():void{
		...
		Alert.show("",0,function(act){
			if(act == "confirm"){
				webService.goRechageFei({uid:server.uid,coinid:_id,phone:_phone},function(response){
					Alert3.show(response.message,null,"common_btn_lab_confirm");
					if(response.code == 0){
						_self.info.status = 5;
						if(_self.info.item){
	...
	private _onAlertAli():void{
		...
		Alert.show("",0,function(act){
			if(act == "confirm"){
				webService.goRechageFei({uid:server.uid,coinid:_id,account:_account,account_name:_name},function(response){
					if(response.code == 0){
						_self.info.status = 4;
						UserData.instance.saveLocalAli(_account,_name);
						Alert3.show("兑换" + _self.info.goodsamount +"元支付宝红包成功",null,"common_btn_lab_confirm");
						if(_self.info.item){
							_self.info.item.updateInfo(_self.info);
						}
						_self.close(true);
	...
	private _initFei(){
		let _self = this;
		let _defNum = UserData.instance.getLocalPhone();
	...
	private _initSpInfo():void{
		...
		this.yidong.addEventListener(egret.TouchEvent.TOUCH_TAP,this._onSelectChannel.bind(this,"cmcc"),this);
		this.liantong.addEventListener(egret.TouchEvent.TOUCH_TAP,this._onSelectChannel.bind(this,"cucc"),this);
		this.dianxin.addEventListener(egret.TouchEvent.TOUCH_TAP,this._onSelectChannel.bind(this,"ctcc"),this);
	}
	...
	private _showFeiKaSp():void{
		...
		let _self = this;
		this["descLabel0"].text = this._buildFeiCardSupport(this["exchangeInfo"])
		this._okFunc = function(){
			if(_self._curSpNum && _self._curSpNum.length == 5){
				webService.goRechageFei({uid:server.uid,coinid:_self.info.id,sp:_self._curSpNum},function(response){
	...
	private _onSelectChannel(sChannel:string):void{
		...
		if(this["exchangeInfo"].support[sChannel] == 1){
			this._feiKaCfg[sChannel].func();
		}else{
			Toast.show(this._buildFeiCardSupport(this["exchangeInfo"]));
		}
	}
	...
	private _defSelChannel():void{
		let _amount = this.info.goodsamount;		
		if(this.info.goodsid == 6){
			if(!this["exchangeInfo"].support.cmcc){
				if(!this["exchangeInfo"].support.cucc){
					this._feiKaCfg["ctcc"].func()
				}else{
					this._feiKaCfg["cucc"].func()
				}
				return;
			}
		}
	...
	private _copyCardInfo():void{
		...
		GameConfig.copyText(this,_str,_str,true);
	}	
	 ...
	 */
	private _initFeiKa(){
		let cfgKey = "red_coin_exchange" + "." + this.info.goodsid + "." + this.info.goodsamount;
		this["exchangeInfo"] = GameConfig.getCfgByField(cfgKey)
	...
	private _initAli(){
		...
		let _self = this;
		let _aliInfo = UserData.instance.getLocalAli();
	...
	private _initDiamond() {
		...
		if(_self.info.status == 3) { // 未领取
			webService.goRechageFei({uid:server.uid,coinid:_self.info.id},function(response){
				_self._onGetCardRet(response);
			});
			return;
		}
		...
		this._exchangeFunc = function () {
			this.dealAction();
			PanelExchange2.instance.show(6, null, {code: _code});
		}

		this._okFunc = 	function(){
			let _code = this.input2.text;	
			GameConfig.copyText(this, _code, _code, false);					
		}
	}
	 ...
	 * 获取兑换话费或者是支付宝信息
	 */
    public static getInstance(): PanelFeiEx {
        if(!PanelFeiEx._instance) {
            PanelFeiEx._instance = new PanelFeiEx();
        }
        return PanelFeiEx._instance;
    }
}