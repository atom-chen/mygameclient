...
class PanelVip extends alien.PanelBase {
    private static _instance: PanelVip;
    public static get instance(): PanelVip {
        if(this._instance == undefined) {
            this._instance = new PanelVip();
        }
    ...
    init():void {
		this.skinName = panels.PanelVipSkin;
	}
	
	createChildren():void {
		super.createChildren();
		this._initEvent();
		this.levelList.itemRenderer = VipLevelItemRender;
		this.goodsList.itemRenderer = VipRewGoodsItemRender;
	...
	private _setStateHasGet():void{
		this["buyImg"].source = "room_getGray";
	...
	private _enableEvent(bEnable:boolean):void{
		...
		server[_func](EventNames.USER_GET_REWARD_REP, this._onRecvGetRewRep, this);
	}
	...
	private _onRecvGetRewRep(e:egret.Event):void{
        ...
        if(data.optype == 9){
            if(data.result == 0 ||data.result == null){
				let tit = "领取VIP奖励成功!";
				if(data.strreward){
					let text = "(您的当前等级为<font color='#E10000'>VIP" + MainLogic.instance.selfData.getCurVipLevel() + "</font>)" ;
					let textFlow = new egret.HtmlTextParser().parser(text);
					let goods = Utils.parseGoodsString(data.strreward)
					PanelItems.instance.show(tit,0x1A1A1A,goods,"",0x7f3000,0,null,"center",textFlow);
				}
	...
	private _formatVipCfg():void{
		...
		let data:any;
		let vipLevel;
		let sendCfg = GameConfig.getCfgByField('friends_cfg.limit');
		this._vipCfg = {nMax:len,info:[],level:{},decline:vipCfg.decline};
		
		let bShowRed = false;
		let sendRedCfg = GameConfig.getCfgByField("webCfg.friendRed");
		if(sendRedCfg && sendRedCfg.status == 1){
			bShowRed = true;
		}


		for(let i=0;i<len;++i){
			let rew = Utils.parseGoodsString(up[i].reward);
			vipLevel = i+1;
			data = {level:vipLevel,exp:up[i].exp,rew:rew,sendNum:sendCfg[vipLevel].giftcnt,showSendRed:bShowRed};
...
class VipLevelItemRender extends eui.ItemRenderer{
	...
	dataChanged():void{
		...
		this.avatar.imageId = MainLogic.instance.selfData.imageid;
		let rew = data.rew;
		let len = rew.length;
		let sText = "";
		for(let i=0;i<len;++i){
			let goodInfo = GoodsManager.instance.getGoodsById(rew[i].id);
			sText += ""+ (i + 1)+".每天领取";
			if(rew[i].id == 2){
				if(rew[i].count % 60 == 0){
		...
		if(data.showSendRed){
			sText += "3.每日可赠送红包" + data.sendNum + "次";
		} 
		this.descLabel.text = sText;
	}
}

class VipRewGoodsItemRender extends eui.ItemRenderer{
