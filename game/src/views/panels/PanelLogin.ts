...
class PanelLogin extends alien.PanelBase {
	private static _instance:PanelLogin;
	public static get instance():PanelLogin {
		if (this._instance == undefined) {
			this._instance = new PanelLogin();
		}
	...
	protected init():void {
		this.skinName = panels.PanelLoginSkin;
	}

	constructor() {
		super(
			alien.popupEffect.Flew, {direction: 'up', ease: egret.Ease.backOut},
			alien.popupEffect.Flew, {direction: 'up', ease: egret.Ease.backIn}
		);
	...
	private onGrpButtonTap(event:egret.TouchEvent):void {
		switch (event.target.name) {
			...
			case 'confirm':
				...
				GameData.instance.setItem('id', id, true);
	...
	show(callback:Function):void{
		...
		this.tiID.text = GameData.instance.getItem('id');
