...
class ScenePlay extends alien.SceneBase {
    ...
    private seat1:SideSeat;// 右边的位置
    private seat2: SideSeat;// 左边的位置
    private seat0:MySeat;
    ...
    private dragonWangZha: DragonGameWangZha;
...
//guesscards
    ...
    private _enterSceneTs: number;
    private _getGameStartTs: number;
    private _delayCheckReconnectTimer:number;
    ...
    private _aniCfg:any = {
        chuntian:{json:"ani_chuntian_json",png:"ani_chuntian_png",w:380,h:230}, //w:404
        feiji:{json:"ani_feiji_json",png:"ani_feiji_png",w:694,h:412},
        zhadan:{json:"ani_zhadan_json",png:"ani_zhadan_png",w:160,h:160},
        wangzha:{json:"ani_wangzha_json",png:"ani_wangzha_png",w:472,h:754},
        shunzi:{json:"ani_shunzi_json",png:"ani_shunzi_png",w:450,h:162},
        liandui:{json:"ani_liandui_json",png:"ani_liandui_png",w:204,h:86},
    }
    constructor() {
        super();
    ...
    private init(): void {
        this.skinName = scenes.ScenePlaySkin;
    ...
    createChildren(): void {
        ...
        if(!server.isMatch && !this.isPersonalGame){
            ...
            /*if(!this.dragonHongBaoWarn)
                this.dragonHongBaoWarn = new DragonHongBaoWarn();*/
        }
        this.dragonChunTian = new DragonGameChunTian();
        this.dragonWangZha=new DragonGameWangZha();
...
//        this.dragonHongBaoWarn.play1();
    ...
    private _onUserOnline(e:egret.Event):void{
        ...
        let _seat:Seat = this.getSeatById(_data.seatid);
        if(!_seat) return;
    ...
    private _onUserOffline(e:egret.Event):void{
        ...
        let _seat:Seat = this.getSeatByUId(_data.uid);
        if(!_seat) return;
    ...
    private _onPlayBrowInfo(fromSeatId:number,toSeatId:number,browId:number){
        let fromSeat:Seat = this.getSeatById(fromSeatId);
        let toSeat:Seat = this.getSeatById(toSeatId);
    ...
    private updateRedCoinInfo(){
        let _selfData = MainLogic.instance.selfData;
        let conf = GameConfig.exchangeConfig.filter((item:any)=>{
            if(item.goodsid==4 || item.goodsid == 1 || item.goodsid == 7 ){
                if(item.hide != 1){
                    if(item.first){
                        let _timeStamp = _selfData.rcminexcexpiretime;//过期时间戳单位秒
    ...
    private onQuitRoomRep(event: egret.Event): void {
        ...
        if(data.result == 0){
            if(data.uid == server.uid){
                ...
                let seat: DDZSeat = this.getSeatByUId(data.uid);
                if(seat) {
    ...
    initData(bOnClickReady:boolean = false): void {
        if(this.isPersonalGame){
            this._seats.forEach((seat: DDZSeat) => {
                seat.initData({ isMatch: false, matchType: 1 })
            });
        }else{
            this._seats.forEach((seat: DDZSeat) => {
                let obj = { isMatch: server.isMatch,matchType: server.roomInfo.matchType ,cleanUserInfo:true,roomType:server.roomInfo.roomType,isSelf:false,roomFlag:this._roomInfo.roomFlag};
    ...
    newGame(): void {
        ...
        server.playing = true;
        this._getGameStartTs = server.getServerStamp();
        if(this._delayCheckReconnectTimer) {
            egret.clearTimeout(this._delayCheckReconnectTimer);
        }
        this._showReadyBtn(false);
        this._showGrpMatchDetail(false);
            ...
            case 'back':
                ...
                this._showPlayGameGetCoin();
                    ...
                    if(this._roomInfo.roomID ==  1000 || this._roomInfo.roomID ==  1004){
                        Alert.show("本局结束即可获得奖杯,现在退出将为您托管游戏", 1, this.onAlertResult.bind(this,0));
                    }                    
                    else{
                        Alert.show(alien.StringUtils.format(lang.backAlert, this.getRedCoinLackCnt()), 1, this.onAlertResult.bind(this,0));
                    }
                } else {

                    if(this._roomInfo.roomFlag == 1){
                        Alert.show("游戏未结束,现在退出将为您托管游戏", 1, this.onAlertResult.bind(this,0));
                    }                    
                    else{
                        Alert.show(alien.StringUtils.format(lang.backAlert1, this.getRedCoinLackCnt()), 1, this.onAlertResult.bind(this,0));
                    }
                }*/
                break;
            case 'continue':
    ...
    private onQuickJoinResponse(event: egret.Event): void {
        ...
        switch(data.result) {
            ...
            case 1:
                ...
                break;
            case 2:
                // Alert.show(lang.noMoreGold,0,this.onAlertResult.bind(this,0));
...
//        data.imageid ="/uploads/avatar/tax465.jpg";
    ...
    private _shouldTipGoldRedRule(){
        let _hasTip = alien.localStorage.getItem("goldRule");
        if(_hasTip != "1"){
            this._showRedNormalRule();
            alien.localStorage.setItem("goldRule","1");
        }
    }
    ...
    private onReconnectRep(event: egret.Event): void {
        ...
        data.players.forEach((player: any) => {
            let seat: DDZSeat = this.getSeatById(player.seatid);
            ...
            if(this._mySeat != seat ) {
                ...
                if(server.isCoupleGame) {
                    ...
                    if(data.mastersid != null && data.mastersid >= 0 && data.mastersid == seat.seatid) {
                        (<SideSeat>seat).setMaster(false, false);
                    }
                    (<SideSeat>seat).addCards(cardids);
                }   
                else {
                    (<SideSeat>seat).setCardCount(player.params[0]);
                }                 
            }
        });
    ...
    private onGetReadyRep(event: egret.Event): void {
        ...
        let seat: DDZSeat = this.getSeatById(data.seatid);
    ...
    private setGiveWayScore(score, masterSeatid,isMaster): void {
        ...
        if(isMaster){
            let graySeat:DDZSeat;
            let normalSeat:DDZSeat;
    ...
    private onSetScore(event: egret.Event): void {
        ...
        //1000 = 极速场
        if(data.ismaster && (!server.isPersonalGame) && (!server.isMatch) && this._roomInfo.roomID == 1001){//只有3人休闲场才可以钻石加倍
   ...
   * }
    ...
    private onUseCard(event: egret.Event): void {
       ...
       /* if(data.lastseatid) {
            if(server.isCoupleGame && data.lastcards) {
                let lastSeat: DDZSeat = this.getSeatById(data.lastseatid);
...
private getGuessCardsType(cards:any):number{
    ...
    private goldfly(cseat:DDZSeat, cx:any, cy:any, tx:any, ty:any): void {
    ...
    private playGuessRightAnimate(seat:any, gold:number): void {
        ...
        if(cseat){
            if(cseat != this._mySeat){
                ...
                //极速场不更新金豆
                if(this._roomInfo && this._roomInfo.roomID != 1000 && !server.isMatch){
    ...
    private _initFiveCard(data:any):void{
        if(data.params && data.params.length >= 2){
            ...
            MainLogic.instance.selfData.setHasPayToday(_todayPayRmb);
    ...
    private _showNewDropCard(pokerId:number,cards:any):void{
        ...
        let proxy = alien.StageProxy;
        let w = proxy.width;
        let h = proxy.height;
        let card:Card = Card.create({pid: 0});
    ...
    private _showFiveCardReward(bShow:boolean,rews:any = null):void{
		...
		if(bShow){
			let _srcY = -24;
            _label.text = "+" + Utils.goodsListToString(rews,0,true);
    ...
    private _openFiveCard(data:any):void{
        for(let i=0;i<5;++i){
            let _card:Card = this["fiveCardContainer"].getChildAt(i);
            if(data[i+1]){
    ...
    private onOperateRep(event: egret.Event): void {
        ...
        if(data.optype == 4){
            ...
            if((data.result == 0 || data.result == null) && data.params && data.params.length >=2){
                ...
                let seat: Seat = this.getSeatById(_seatId);
                if(!seat) return;
        ...
        if(this._mySeat.seatid == data.seatid) {
            ...
            if(data.seatid == data.pseatid || data.pseatid == 0) {
                ...
                if(!canUse) {  //如果没有可出的牌,倒计时变为5秒
                    ...
                    if(cardType == SHUANGWANG) { //王炸时倒计时变为2秒
                        if(timeout > 1){
                            timeout = 1;
                        }
                    }
    ...
    private onMatchInfoNtf(event: egret.Event): void {
        ...
        switch(data.optype) {
            case 101:
                // recv:.user.MatchInfoNtf {"matchid":401,"clientid":1,"optype":101,"timeout":null,"params":[39,48,-45,120,48,18,0],"rankInfo":[],"roundInfo":[],"resultInfo":[],"strreward":null,"roundscore":[]}
     ...
     */
    private _changeEffectEnd(seat:DDZSeat,change:number,kb:number):void{
        ...
        if(this._roomInfo.roomFlag == 2){
            ...
            if(seat)
                seat.updateGold(change, server.isMatch);
        }
     ...
     * 播放本局的加减分动画
     */
    playChangeEffect(_seatIdx:number, seat:DDZSeat,change:number,kb:number, delay:number = 0):void {
        let score = change + kb;
     ...
     * 显示破产或者是封顶
     */
    private _showMaxOrZero(seat:DDZSeat,change:number,kb:number):void{
        if(!server.isMatch && !server.isPersonalGame){
            ...
            if(change > 0){
                ...
                if(_showMax){
                    ...
                    _btnBgImg.source = "common_btn_blue_h";
                    _btnLabel.text = "封顶";
                ...
                if(_userHas <= 0){
                    ...
                    _btnBgImg.source = "common_btn_orange_h";
                    _btnLabel.text = "破产";
    ...
    showResult1(success: boolean,_myCurGold:number):void{
        ...
        if(server.isMatch || server.isPersonalGame){
            ...
            if(success){
                ...
                }else if(roomId == 8003 || roomId == 8004 || roomId == 8000){ //钻石中级和高级
                    this._initNewRedProcess();
                }
            }
        }
    ...
    private onUpdateGameInfo(event: egret.Event): void {
        ...
        if(msg.params1.length >=4 && !server.isMatch){
            if(msg.params1[3]){
    ...
    private _goldSmallMinScore():void{
        PanelExchange2.instance.show(0,()=>{
            ...
            if(selfGold < this._roomInfo.minScore){
                ...
                Alert.show("是否继续进行游戏！",1,(action: string) => {
                    if(action == 'confirm') {
                        this.initData();
                        this.sendQuickJoin();
                    } else {
        ...
        switch(data.result) {
            case 0:   //成功
                ...
                if(this._roomInfo.roomID == 1001 || this._roomInfo.roomID == 1005){ //只有金豆场和大师场才更新金豆
                    this._mySeat.updateGold(GameConfig.almsConfig.gold,false);
                }
                //救济金返回后还未同步到玩家身上
	 ...
	 * @returns {MySeat}
	 */
    private playerSeatDown(data: any): DDZSeat {
        if(data.uid == server.uid) {
        ...
        if(!seat || !seat.userInfoData || seat.userInfoData.uid != data.uid) {
            ...
            seat = this._seats[(userInfoData.seatid + seatNum - server.seatid) % seatNum];
    ...
    private playerStandUp(uid: number): void {
        ...
        let seat: DDZSeat = this.getSeatByUId(uid);
        //一局结束,当玩家离开不清玩家信息
	 ...
	 * @returns {Seat}
	 */
    private getSeatByUId(uid: number): DDZSeat {
        let seat: DDZSeat = null;
        this._seats.some((s: DDZSeat): boolean => {
    ...
    private onLotteryRedCoinRep(e:egret.Event):void
    {
        if (e.data.result==0){
            ...
            //钻石场要更新进度
            if(this._roomInfo.roomFlag ==2 && !server.isMatch && !server.isPersonalGame){
                this._initNewRedProcess(); 
            }
        }
    }
     ...
     *  state: 某个座位
     */
    private _playAniByActName(actName:string,state:string = "my"){
        let data = this._aniCfg[actName]
		let mcData = RES.getRes(data.json);
		let pngData = RES.getRes(data.png);
		var mcDataFactory = new egret.MovieClipDataFactory(mcData, pngData);
		var role:egret.MovieClip = new egret.MovieClip(mcDataFactory.generateMovieClipData(actName));
		
        role.addEventListener(egret.Event.COMPLETE,  (e:egret.Event)=> {
            this.removeChild(role);
            if(actName == "chuntian"){
                //this._setShowChunTian(false);
            }
        },this);
        
        role.anchorOffsetX = data.w * 0.5;
        role.anchorOffsetY = data.h * 0.5;
        let a = role.$getContentBounds();
        if (state != ""){
            if (state == "my"){
                role.x = alien.StageProxy.stage.stageWidth *0.5;
                role.y = alien.StageProxy.stage.stageHeight *0.5;
            }else if(state == "right"){
                role.x = this.seat1.x - 350;
                role.y = this.seat1.y + 100;
                if(this.currentState == "couple"){
                    role.x = this.seat1.measuredWidth - 350;
                }
            }else if(state == "left"){
                role.x = this.seat2.x + 350;
                role.y = this.seat2.y + 100;
            }
        }
        this.addChild(role)
		role.gotoAndPlay(1);
    ...
    private _onClickFiveRule():void{
        let _dropRew = GameConfig.getCfgByField("item_drop");
        let _formatInfo = function(rewObj:any){
            let _info = Utils.parseGoodsString(rewObj.reward);
            let _text = Utils.goodsListToString(_info,0,true);
            return _text;
        }
    ...
    private _showNotPayTodayForGetFiveRew():void{
        Alert.show("充值任意金额，即有机会获得最高500倍奖励！确定前往充值？",0,function(act){
            if(act == "confirm"){
                PanelExchange2.instance.show(0);
            }
        });
    ...
    private _onClickOpenFiveCard():void{
        if(MainLogic.instance.selfData.hasPayToday()){
            this._showFiveOpenCard(false);
            server.reqGetFiveCardRew(this._roomInfo.roomID);
        }else{
            this._showNotPayTodayForGetFiveRew();
        }
    }
    ...
    private _addClickFunc():void{
        ...
        this.match9Rule_img[func](this._onClickMatch9Rule,this);
        this.matchInfo_img[func](this._onClickMatch9Info,this);
        this.match9RuleAniClose_btn[func](this._onCloseMatch9RuleAni,this);
        ...
        this["grpMatchDetail"][func](this._onClickGrpMatchDetail, this);
        this["btnAbandon"][func](this._onClickAbandon, this);

        this['btnTest'][func](this._onClickTest, this);
        this['btnTest2'][func](this._onClickTest2, this);
        this['btnTest3'][func](this._onClickTest3, this);
    }

    private _onClickTest(evt: egret.TouchEvent): void {
        server.getMatchSignUpInfo(303)           
    }

    private _curRound1 = 1;
    private _onClickTest2(evt: egret.TouchEvent): void {        
        server.sendRedcoinRankingListReq(1, 303, this._curRound1, 1);
        this._curRound1++;
        if(this._curRound1 > 6) {
            this._curRound1 = 1;
        }
    }

    private _curRound2 = 1;
    private _onClickTest3(evt: egret.TouchEvent): void {        
        server.sendRedcoinRankingListReq(2, 303, this._curRound2, 1);
        this._curRound2++;
        if(this._curRound2 > 6) {
            this._curRound2 = 1;
        }
    }

    beforeShow(params: any): void {
        console.log("scenePlay--beforeShow----->", params);
        ...
        if(params.personalgame){
            ...
            if (!roomInfo) { 
                ...
                roomInfo = GameConfig.getRoomCfgByGameId(params.gameID, params.roomID);
                if(!roomInfo) {
                    MainLogic.backToRoomScene();
                    return ;
                }
            ...
            /*zhu 暂时去掉if(!this._roomInfo || !this._roomInfo.roomID || !this._roomInfo.roomType){
                webService.postError(ErrorConfig.REPORT_ERROR,"ScenePlay beforeShow=>"+ version + "|uuid" + webService.uuid +"|" +JSON.stringify(params) + JSON.stringify(GameConfig.roomList));
            }*/
            server.isMatch = roomInfo.roomType == 2;

            let  currentState = server.isMatch ? 'match' : 'normal'; 
            if(roomInfo.minPlayer == 2) {
                this.isCoupleGame = true;
                server.isCoupleGame = true;
                currentState = "couple";
            ...
            if(!server.isMatch){
                ...
                this._showFiveCardReward(false);
                this._clearFiveCardView();
                server.reqFiveCard(this._roomInfo.roomID);
                this._initNewRed();
    ...
    private _onDayTaskRewGetSucc():void{
        let _num = MainLogic.instance.selfData.getDayTaskOverCount();
...
//            this._mySeat.setOwnerFlagVisible(true);
    ...
    protected onShow(params: any = null,back: boolean = false): void {
        ...
        if(this._roomInfo && this._roomInfo.minPlayer == 2) {
            this.isCoupleGame = true;
            server.isCoupleGame = true;
        ...
        if(server.isMatch) {     
            this.btnRecord.visible = false;     
            console.log("scenePlay----onshow----------->", params, this._roomInfo);    
            let _roomid = 0;
            if(!params.roomID) {
            ...
            this._enterSceneTs = server.getServerStamp();
            this._getGameStartTs = 0;            
            this._delayCheckReconnectTimer = egret.setTimeout(()=>{
                if(this._getGameStartTs == 0) {
                    console.log("server.checkReconnect()-------_delayCheckReconnectTimer---->")
                    server.checkReconnect();
                }                
            }, this, 18000);
        ...
        //极速场或者是金豆赢一局抽奖券
        else if(server.roomInfo && (server.roomInfo.roomID == 1000|| params.roomID == 1004)){
            this.setlabWaiting('正在为您匹配对手...');
        }
    }
...
//        alien.SoundManager.instance.stopMusic();
    ...
    private _onClickRed():void{
        ...
        let _ins = ExchangeService.instance;
    ...
    private _onUserRedCountChange(e:egret.Event):void{
        let roomId = this._roomInfo.roomID;
        if(server.isMatch || !roomId || roomId == 1000 || roomId == 1004){
            return;
        }
        let _data = e.data;
        let _bShow = false;
        let winCntInfo = MainLogic.instance.selfData.getRoomWinCntInfo(roomId);
    ...
    private _checkShowFailDouble():any{
        let _seat:DDZSeat = this._mySeat;
        let _rightSeat:DDZSeat = this['seat1'];
        let _leftSeat:DDZSeat = this['seat2'];        
        let leftGold = 0;
    ...
    private _onServerDoubleRep(_data):void{
        if(_data.params && _data.seatid && _data.params.length>=1){
            ...
            let seat: DDZSeat = this.getSeatById(_data.seatid);
            if(!seat) return;
    ...
    private _isEnoughChip():boolean{
        let bEnough = false;
        let gold = MainLogic.instance.selfData.gold;
        ...
        if(this._roomInfo.roomFlag == 1){
            ...
            let diamond =BagService.instance.getItemCountById(3);
    ...
    public checkEnoughSendQuickJoin():void{
        if(this._isEnoughChip()){
            ...
            let _gold = MainLogic.instance.selfData.gold;
            let _room = GameConfig.getSuitableRoomConfig(_gold);
            ...
            if(!_room){ //未找到合适的房间
                ...
                MainLogic.instance.toSuitableRoom(_room);
                server.checkReconnect();
                return;
            }else{
                this.sendQuickJoin();
    ...
    */
    ...
    private _initNewRed():void{
        if(server.isPersonalGame || server.isMatch){
            return;
        }
        //钻石场
        if(this._roomInfo.roomFlag == 2){
    ...
    private _onClickNewRed():void{
        let roomId = this._roomInfo.roomID;
        let winCnt:any = MainLogic.instance.selfData.getRoomWinCntInfo(roomId);
        let sText:string;
        if(this._roomInfo.roomFlag == 1){
            if(roomId == 1001||roomId == 1002||roomId == 8001 || roomId == 1006){
                this._showNormalRedImg(false);
                ExchangeService.instance.doChou(roomId);
                return;
            }else{
                //金豆场时间到了才可以抽
                if(this["canNewRed"]){
            ...
            if(roomId == 8000 || roomId == 8003 || roomId == 8004){ //钻石中级和高级
                this._showNormalRedImg(false);
                ExchangeService.instance.doChou(roomId);
                return;
            }else{
    ...
    private _setTableRedInfo():void{
        ...
        let sText = "";
        let roomId = this._roomInfo.roomID;
        let cnt:number = MainLogic.instance.selfData.getRoomRedWinCnt(roomId);
        ...
        if(winCnt){
            if(this._roomInfo.roomFlag == 1 || this._roomInfo.rewardwinround){
                ...
                if(roomId == 1000){
                    ...
                    cnt = (this._roomInfo.rewardround || 1) - (winCnt.cnt||0);
                }
    ...
    private _tipNewRedInfo():void{
        let  sText:string = "";
        //let winCnt:any = MainLogic.instance.selfData.getRoomWinCntInfo(this._roomInfo.roomID);
        let info = this._getNeedPlayInfo();
        if(this._roomInfo.roomFlag == 1){
        ...
        if(this._roomInfo.roomFlag == 1){
            ...
            if(winCnt){
                if(winCnt.time && winCnt.chance >= 1){
                    if(server.tsServer>= winCnt.time){
                        this["canNewRed"] = true;
                        this._onHasRedChance();
                    }
                }
            ...
            if(winCnt){
                ...
                if(roomId == 1000){
                    if(winCnt.getchance < 2 ){
                        ...
                        if(cnt > this._roomInfo.rewardround){
                            cnt = this._roomInfo.rewardround;
                        }
                        obj.text = cnt + "/" + this._roomInfo.rewardround;
                    }
                }else{
                    if(cnt >= this._roomInfo.rewardround){
                        cnt = this._roomInfo.rewardround;
                ...
                if(roomId == 1000){
                    ...
                    obj.text = "1/" + this._roomInfo.rewardround;
                }
            }
    ...
    private _getNearestRedOpenTime():any{
        let nCur = alien.Utils.getCurTimeStamp();
        ...
        if(data.optype == 1){ //抽到的红包金额
            ...
            /*if(data.result == null){
                ...
                Alert.show(_str,0,function(){
                    if(!server.playing && this._roomInfo && this._roomInfo.roomID== 1000){ //极速场 
                        let _nDiamond = BagService.instance.getItemCountById(3);
                        if(this._roomInfo.minScore >_nDiamond){
                            MainLogic.instance.ifBuyReviveMaxTipShop(this._roomInfo.minScore);
                            return;
                        }
                        else{
                            this.sendQuickJoin();
    ...
    private _showRedNormalRule():void{
        ...
        if(this._roomInfo.roomID == 1000){
           _text = "福袋规则：不限时3局场，不限制时间，只要完成3局就能福袋抽奖杯。\n"
            ...
            +"        2）钻石不足赔付的处理：您（玩家）拥有多少钻石，只能产生这些钻石的输赢，系统不额外抽取，也不额外赔偿。\n"
            +"服务费：每局游戏会收取一定的服务费。\n"
        }else if(this._roomInfo.roomID == 8000){
            _text = "福袋规则：不限时2局场，不限制时间，只要玩2局就能福袋抽奖杯。\n"           
            +"特殊情况：\n"
            +"        1）中途退出游戏：不限时2局场。\n"
            +"        2）钻石不足赔付的处理：您（玩家）拥有多少钻石，只能产生这些钻石的输赢，系统不额外抽取，也不额外赔偿。\n"
            +"服务费：每局游戏会收取一定的服务费。\n"
    ...
    private _onCloseRevivePanel():void{
        //金豆场关闭复活礼包后在请求救济金
        if(!server.isMatch && !server.isPersonalGame && this._roomInfo.roomFlag != 2){
            if(!MainLogic.instance.alms()) {
                MainLogic.backToRoomScene();
            } 
        }else{
            MainLogic.backToRoomScene();
