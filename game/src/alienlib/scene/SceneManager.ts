...
module alien {
	export class SceneManager {
		...
		static init(root:eui.Group, loadingSceneName:string = "loading"):void {
			SceneManager.instance.init(root, loadingSceneName);
		}

		static addMostTopScene(name:string, sceneDef:any):void{
			SceneManager.instance.addMostTopScene(name, sceneDef);
		}

		static addTopScene(name:string, sceneDef:any):void{
			SceneManager.instance.addTopScene(name, sceneDef);
		}

		static addEffectScene(name:string, sceneDef:any):void{
			SceneManager.instance.addEffectScene(name, sceneDef);
		}
		...
		static register(name:string, scene:Object, resGroup:string = null):void {
			SceneManager.instance.register(name, scene, resGroup);
		}
		...
		static show(name:string, params:any = null, effectDef:any = null, effectParams:any = null, callback:Function = null, back:boolean = false, setLastSceneName:string = null):void {
			SceneManager.instance.show(name, params, effectDef, effectParams, callback, back, setLastSceneName);
		}

		static back(params:any = null, effectDef:any = null, effectParams:any = null, callback:Function = null):boolean {
			return SceneManager.instance.back(params, effectDef, effectParams, callback);
		}
		...
		static addOtherContainer(container:any):void{
			if(!container) return;
			SceneManager.instance.topLayer.addChild(container);
		}
		...
		static removeOtherContainer(container:any):void{
			if(container){
				SceneManager.instance.topLayer.removeChild(container);
			}
		}
		...
		static removeDDZContainer():void{
			...
			let itemScene:alien.SceneBase;
			_middleLayer.$children.forEach((item)=>{
				itemScene = <alien.SceneBase>item;
				if(!(itemScene instanceof SceneLoading)){
					ins.mapScene.remove(itemScene.name);
					alien.EventManager.instance.disableOnObject(itemScene);
		...
		/*static showLoading(bShow:boolean):void{
			let _ins = SceneManager.instance;
			if(_ins.loadingSceneName){
				let _scene:SceneLoading = _ins.mapScene.get(_ins.loadingSceneName);
				if(_scene){
		...
		*/
		...
		private onStageResize(event:egret.Event = null):void{
			alien.Utils.enumChildren(this.root, (child:egret.DisplayObject)=>{
		...
		reDrawBG(){
			...
			g.drawRect(0, 0, alien.StageProxy.width, alien.StageProxy.height);
			g.endFill();
		}
		...
		private onResourceProgress(event:RES.ResourceEvent):void {
			if (event.groupName == this.currentLoadGroup) {
				alien.Dispatcher.dispatch('loadingProgress', {
        ...
        private onResourceLoadError(event: RES.ResourceEvent): void {
            webService.postError(ErrorConfig.RESOURCE_LOAD_ERROR,event.groupName);
        }
