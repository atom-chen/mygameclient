...
module alien {
	...
	export class TimeUtils {
        ...
        static timeFormatForEx(date: Date,sYear:string ="年",sMonth:string ="月",sDay:string ="日",withHourMin:boolean = true,withSec:boolean = false):string { 
		 ...
		 if(withHourMin){
			...
			if(withSec){
				return str + " " + hours + ":" + minutes + ":" + date.getSeconds();
			}
		...
		static dateTimeToString(date:Date):string{
			return this.dateToString(date, lang.format_y_m_d) + ' ' + this.dateToTimeString(date, '{2}:{1}');
		}
		...
		static tsToMonthDayString(ts:number):string {
			...
			return StringUtils.format('{0}' + lang.month + '{1}' + lang.day, StringUtils.supplement(date.getMonth() + 1, 2), StringUtils.supplement(date.getDate(), 2))
		}
