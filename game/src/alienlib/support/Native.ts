...
let _nameLen = 6;
let _idLen = 18;
let _phonePrompt = "请输入"+ _phoneLen + "位手机号码";
let _codePrompt = "请输入" + _codeLen +"位数字验证码";
let _namePrompt = "请输入姓名";
let _idPrompt = "请输入18位身份证号码";
let _exCodePrompt = "请输入兑换码";
...
module alien {
    export class Native extends egret.EventDispatcher {
        ...
        get platformId(): number {
            return alien.Native.instance.isNative ? (egret.Capabilities.os == 'iOS' ? 2 : 3) : 4;
        }
        ...
        isLoginByWXQr():boolean{
            let wxqr = (alien.localStorage.getItem("wxqr") == "1")
            return wxqr;
        }
        ...
        isLoginByAliQr():boolean{
            let aliqr = (alien.localStorage.getItem("aliqr") == "1")
            return aliqr;
        }
        ...
        public setChannelName(sName:string):void{
            ...
            if(sName == "wxmp"){
                ...
                this._isAli = true;
                alien.localStorage.setItem("aliqr","1");
                alien.localStorage.setItem("wxqr","0");
            }else if(sName == "wxqr"){
                this._isWxQr = true;
                alien.localStorage.setItem("wxqr","1");
                alien.localStorage.setItem("aliqr","0");
            }else if(sName == "aliqr"){
                this._isAliQr = true;
            }
        }
        ...
        call(method: string,args: any = null,callback: Function = null): void {
            args = args || {};
            let params: any = {
                id: alien.StringUtils.makeRandomIntString(10),
                method,
                args
            };
        ...
        getDeviceUUID(callback: Function,prefix: string,makeNew: boolean = false): void {
            let uuid: string = alien.localStorage.getItem('uuid',prefix);
            if(makeNew || !uuid) {
                uuid = alien.StringUtils.makeRandomString(32);
                alien.localStorage.setItem('uuid',uuid,prefix);
            }
            callback({ uuid });
        }
        ...
        openUrlByWebview(sUrl:string){
            this.call("openUrlByWebView",{url:sUrl,left:10,top:10,maxWidth:alien.StageProxy.width-20,maxHeight:alien.StageProxy.height-20},null);
        }
        ...
        getAndroidLocalVercode(){
            return Number(alien.localStorage.getItem("apkDownVerCode"));
        }
        ...
        setAndroidLocalVercode(verCode:number){
            alien.localStorage.setItem("apkDownVerCode","" + verCode);
        }
         ...
         */
        getAndroidDownState(){
            return Number(alien.localStorage.getItem("apkDownState"));
        }
        ...
        setAndroidDownState(state:number){
            alien.localStorage.setItem("apkDownState","" + state);
        }
       ...
       /**
        ...
        rechageWXApp(response: any,callback:Function = null){
            this.call("payByWechat",response,function(ret){
                ...
                if(ret.code ==0){
					...
					BagService.instance.refreshBagInfo(true);
                }
            });
        }
        ...
        rechargeAliH5(response:any,callback:Function = null):void{
            ...
            let _stage = alien.StageProxy.stage;
            ...
            */
            if(alien.Native.instance.isNative){
                let payUrl = GameConfig.getCfgByField("custom.payUrl");
                if(payUrl){
                    let url = payUrl + "?ali=1&info="+_alipay + "&time=" + new Date().getTime();
                    GameConfig.toUrl(url);
                }
                return;
            }

            let _base64Src = Base64.decode(_alipay);
	...
	public checkAppUpdate(noUpdateCb:Function){
        ...
        if(_platform == "Android"){  
            _bAndroid = true;
            _nativeBridge.getAndroidExtPath(function(sFullPath){
		        let _tmp = "tmp.apk";
		...
		webService.checkAppUpdate(function(bHasNew,jInfo){
			//无版本更新
			if(!bHasNew){
                if(_bAndroid){
				    _nativeBridge.clearAndroidDown(_sTmpFull,_sFinailFull);
                }
				noUpdateCb();
			}else{
                //非安卓直接返回
                if(!_bAndroid){
                    ...
                    if(_self._channelId == "20000"){
                        _nativeBridge.openUrlBySys(jInfo.url);
                    }
                    return;
                }
                ...
                let _progress = function(_jPro){
                    ...
                    SceneLoading.setLoadingText("下载更新中。。。" + _number + "/" + 100);
                }

                let _finish = function(sInfo){
                    SceneLoading.setLoadingText("下载更新完成,请稍候。。。");
                    _nativeBridge.checkMd5(_sFinailFull,function(sMd5){
                        console.log("downFinish======>",sMd5,jInfo.md5,_retryCount,_maxRetry);
                        if(_retryCount >= _maxRetry){
                            Alert.show("您的网络不稳定,请切换网络环境重试",0,function(action){
                                if(action=="confirm"){
                                    _retryCount = 1;
                                    _nativeBridge.downApk(jInfo.version,jInfo.url,_sTmpFull,_sFinailFull,jInfo.cleanTmp,_progress,_finish);
                                }else{
                        ...
                        if(jInfo.md5 != sMd5){
                            ...
                            _nativeBridge.installApk(_sFinailFull);
                        }
                    });
                }
...
window["onAndroidKeyBack"] = function(){
    alien.Native.instance.dispatchEventWith("back",false,null);
}
...
window["onAndroidPause"] = function(){
    ...
    let _soundManager = alien.SoundManager.instance;
