class RunFastCardsHint {
	...
	static getCardsHash(cards: Array<number>): {} {
		...
		for (var i = 0; i < cards.length; ++i) {
			key = RunFastCardsHint.getCardLogicValue(cards[i]);
			if (!hash[key]) {
				hash[key] = [cards[i]];
			} else {
	...
	static getAllFourCard(cards: Array<number>, max_amount: number = 0) {
		...
		for (var k in hasTab) {
			if (hasTab[k].length == 4) {
				...
				// 		ans = { node: RunFastCardsType.GetType(tc), cards: tc };
				...
				if (max_amount < 4) {
					ans = { node: RunFastCardsType.GetType(hasTab[k]), cards: hasTab[k] };
					max_amount = 4;
				}
			}
		}
			...
			for (var j = i + 1; j < 13; ++j) {
				if (hasTab[j]) {
					...
					if (j == 12 && line.length > 4 && line.length > max_amount) {
						ans = { node: RunFastCardsType.GetType(line), cards: line };
	...
	static getMaxAmountCards(cards: Array<number>): any {
		...
		var temp = RunFastCardsType.GetType(cards)
		if (temp.ct > 0) {
			return cards;
		}

		temp = RunFastCardsHint.getAllCardsPossible(cards);
	...
	static getFitCards(cards: Array<number>, idx: number, precards: Array<number> = []): any {
		...
		if (precards.length > 0) {
			prenode = RunFastCardsType.GetType(precards);
