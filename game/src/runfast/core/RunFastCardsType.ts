let RunFastBUYAO: number = 0;
let RunFastSHUANGWANG: number = 1;
let RunFastZHADAN: number = 2;
let RunFastDAN: number = 4;
let RunFastDUIZI: number = 5;
let RunFastSANZHANG: number = 6;
let RunFastSANDAIYI: number = 7;
let RunFastSANDAIYIDUI: number = 13;
let RunFastSIDAIER: number = 8;
let RunFastSIDAIERDUI: number = 14;
let RunFastFEIJI: number = 9;
let RunFastDANSHUN: number = 10;
let RunFastSHUANGSHUN: number = 11;
let RunFastSANSHUN: number = 12;
let RunFastSANDAIER: number = 15;//pdk
let RunFastSANTIAOA: number = 16;//pdk
let RunFastSIDAISAN: number = 17;

class RunFastCardsType {
	static CheckType(cards: any[], targetCards: any[], totalCards: any[] = []): boolean {
		...
		if (result1.ct == RunFastSHUANGWANG) return true;
		if (result1.ct == RunFastSANTIAOA) return true;
		if (targetCards == null || targetCards.length < 1) {
			//最后一把可以出三带一
			// if (result1.ct == RunFastSANDAIYI && totalCards.length != 4) {
			// 	return false;
			// }
			return result1.ct != 0;
		}
		...
		// //最后一轮才能出三带一
		// if ((result2.ct == RunFastSANDAIYI || result2.ct == RunFastSANDAIER) && result1.ct == RunFastSANDAIYI) {
		...
		} else if (result2.ct != RunFastSHUANGWANG) {
			if (result2.ct != RunFastZHADAN && result1.ct == RunFastZHADAN) {
				return true;
			}
		}
	...
	static m_getCards(targetCards: any[], type: number, out1Arr: any[], out2Arr: any[], out3Arr: any[], out4Arr: any[], totalCards: any[], targetType: number): Object {
		...
		if (type == RunFastSHUANGWANG || type == 0) return null;
		if (type == RunFastSANTIAOA || type == 0) return null;
		// if (type == RunFastSANDAIER && totalCards.length == 4) return null;
		...
		if (type == RunFastSANDAIER && totalCards.length == 4 && tmp4Arr.length < 1) return null;
		if (type == RunFastDAN) {
		...
		if (type == RunFastFEIJI) {
			...
			if (count >= 2) {
				...
				console.log("pdk's feiji-------->", targetCards, targetType);
		...
		if (type == RunFastZHADAN) {
			if (tmp4Arr.length > 0) {
				// console.log("RunFastZHADAN--------->", tmp4Arr, out4Arr);
				for (i = 0; i < tmp4Arr.length; ++i) {
					if (tmp4Arr[i] > out4Arr[0]) {
						tmp = [tmp4Arr[i], tmp4Arr[i], tmp4Arr[i], tmp4Arr[i]];
						if (type == targetType) {
	...
	static GetType(cards: any[], out1Arr: any[] = null, out2Arr: any[] = null, out3Arr: any[] = null, out4Arr: any[] = null): any {
		...
		//单牌
		if (cards.length == 1) {
			return { ct: RunFastDAN, kv: tmp1Arr[0] };
		}
		//双王
		if (cards.length == 2 && cards.indexOf(140) >= 0 && cards.indexOf(150) >= 0) {
			return { ct: RunFastSHUANGWANG, kv: 15 };
		}
		//炸弹
		if (tmp4Arr.length == 1 && cards.length == 4) {
			return { ct: RunFastZHADAN, kv: tmp4Arr[0] };
		}
		//AAA炸弹
		if (tmp3Arr.length == 1 && cards.length == 3 && tmp3Arr[0] == 12) {
			return { ct: RunFastSANTIAOA, kv: tmp3Arr[0] };
		}
		//对子
		if (tmp2Arr.length == 1 && cards.length == 2) {
			return { ct: RunFastDUIZI, kv: tmp2Arr[0] };
		}
		//三张
		if (tmp3Arr.length == 1 && cards.length == 3) {
			return { ct: RunFastSANZHANG, kv: tmp3Arr[0] };
		}
		//三带一
		if (tmp3Arr.length == 1 && cards.length == 4) {
			return { ct: RunFastSANDAIYI, kv: tmp3Arr[0] };
		}
		//三带对
		if (tmp3Arr.length == 1 && tmp2Arr.length == 1 && cards.length == 5) {
			//return {ct:RunFastSANDAIYIDUI, kv:tmp3Arr[0]};			
			return { ct: RunFastSANDAIER, kv: tmp3Arr[0] };
		}
		//三带二
		...
		// 	return { ct: RunFastSIDAIER, kv: tmp4Arr[0] };
		// }
		//四带二
		...
		if (tmp4Arr.length == 1 && cards.length == 7) {
			return { ct: RunFastSIDAISAN, kv: tmp4Arr[0] };
		}
		// //四带对
		// if(cards.length == 8){
		// 	if(tmp4Arr.length == 1 && tmp2Arr.length == 2){
		// 		return {ct:RunFastSIDAIERDUI, kv:tmp4Arr[0]};
		// 	}else if(tmp4Arr.length == 2){
		// 		return {ct:RunFastSIDAIERDUI, kv:tmp4Arr[1]};
		// 	}
		// }
		//飞机
		...
		if (tmp3Arr.length > 1) {
			...
			if (tmp.result) {
				...
				if (tmp3Arr.length * 3 == cards.length) {
					return { ct: RunFastSANSHUN, kv: tmp.kv };
				}
			}
		}
		//单顺
		if (tmp1Arr.length > 4 && cards.length == tmp1Arr.length) {
			...
			// console.log("RunFastDANSHUN----tmp---111---->", tmp)
			if (tmp.result) {
				return { ct: RunFastDANSHUN, kv: tmp.kv };
			}
			//特殊处理A2345
			...
			// 	// console.log("RunFastDANSHUN----tmp---222---->", tmp)
			// 	if (tmp.result) {
			...
			// 	tmpCards.splice(indexOf2);
			// 	tmp = this.is_shun(tmpCards);
			// 	// console.log("RunFastDANSHUN----tmp---333---->", tmp)
			// 	if (tmp.result) {
			// 		if (tmp.kv - tmpCards.length == 0) {
			// 			return { ct: RunFastDANSHUN, kv: tmp.kv };;
			// 		}
		...
		if (tmp2Arr.length >= 2 && tmp2Arr.length * 2 == cards.length) {
			...
			if (tmp.result) {
				return { ct: RunFastSHUANGSHUN, kv: tmp.kv };
			}
		}
