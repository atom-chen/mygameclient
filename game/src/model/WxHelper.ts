...
class WxHelper {
	...
	{
        if(!alien.Native.instance.isWXMP) return;

        let userData: UserData = UserData.instance;
        var sk: string = userData.getSk();
        if(!sk){
            sk = "";
            Reportor.instance.reportCodeError("sk error share :" + alien.Native.instance.getChannelName() + "isApp:"+alien.Native.instance.isNative);
        }
        //link1:分享到朋友圈
        //link2：分享给朋友
        //imgUrl：图片链接
        ...
        if (type == this.SHARE_FREE_GAME)
        {
            ...
            shareParams.desc2 = alien.StringUtils.format(lang.thirdPartWxMp.desc3,parmas[2],parmas[0],parmas[1]);
            shareParams.link2 = GameConfig.WX_LOGIN_URL + "?state=3&share_room="+parmas[2];
        }
        else
        {
            ...
            shareParams.link2 =  GameConfig.WX_LOGIN_URL  + "?state=3&"+"share_sk=" + sk;//lang.thirdPartWxMp.share_app_url + "?share_sk=" + sk;
        }
        
        alien.Native.instance.share(shareParams,callBack);
	}
    ...
    private static _onShareRet(response):void{
        ...
        alien.Dispatcher.dispatch(EventNames.USER_SHARE_RESULT,{shareSucc:_bSucc});
    }
    ...
    public static doWebH5ShareCfg():void{
         WxHelper.share(2,WxHelper._onShareRet.bind(WxHelper));
    }
    ...
    public static doNativeShare():void{
        WxHelper.shareForNative(2,WxHelper._onShareRet.bind(WxHelper));
    }
	...
	public static shareForNative(type:number, callBack:any, parmas:any[]=[]):void {
        ...
        let shareH5Url = GameConfig.WX_LOGIN_URL + "?state=3&" + "share_sk=" + sk;
        ...
        if (type == this.SHARE_FREE_GAME) {
            ...
            shareParams.description = alien.StringUtils.format(lang.thirdPartWxMp.desc3, parmas[2], parmas[0], parmas[1]);
            shareParams.url = GameConfig.WX_LOGIN_URL + "?state=3&share_room=" + parmas[2];
        }
    ...
    public static shareImageNative(platform:number, callBack:any, parmas:any[]=[]): void {
        ...
        // egret.log('alien.Native.instance.share');
        alien.Native.instance.share(shareParams, callBack);
    }
    ...
    static shareForPhone():void{
        webService.getShareImgUrlByScene(function(response){
