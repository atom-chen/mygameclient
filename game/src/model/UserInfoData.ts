...
//	losecnt:number;
	...
	public setHasSendNum(num):void{
		...
		alien.Dispatcher.dispatch(EventNames.FRIEND_SEND_CHAGNE);
	}
	...
	public calculateLeftSendNum():void{
		...
		if(vipLevel >=1){
			let limitCfg = GameConfig.getCfgByField('friends_cfg.limit');
	...
	private _friendsReqChange():void{
		...
		alien.Dispatcher.dispatch(EventNames.FRIEND_REQ_CHAGNE,{num:num});
	}
	...
	public isMaxBuyGoldRevive():boolean{
		let _maxBuy = GameConfig.getGoldReviveMaxBuy();
	...
	initData(data:any):void{
		...
		alien.Utils.injectProp(this, data, (target, key, value)=>{
			if(key == 'nickname'){
				if(value){
					if(value != "参赛玩家"){
						target.nickname = Base64.decode(value);
					}
					else{
				...
				UserInfoData.updateSex(target, value);
			}else if(key == 'gold'|| 
				key == "redcoingot" ||
				key == "totalwincnt" || 
				key == "totallosecnt" ||
				...
				if((value >= 1 && value <=9) || (value == -1)){
					target[key] = parseInt(value);
					if(value != -1){
						alien.Dispatcher.dispatch(EventNames.FRECHARGE_HASUPDATE);
					}
				}
			...
			else if(key == 'wincnt')
            {
				...
				if(value &&value.length >= GameConfig.getDayTaskList().length){
					target[key] = value;
...
//        } 
	...
	static updateSex(target:UserInfoData, sex:number = 0):void{
		target.sex = sex;
	...
	private _calculateOverTaskNum(val:any):void{
		let _taskCfg = GameConfig.getCfgByField("newTaskInfo");
		let _num:number = 0;
		...
		if(!GameConfig.fishTask || !GameConfig.fishTask.length) {
		...
		if(_lastNum != this._dayTaskOverNum){
			alien.Dispatcher.dispatch(EventNames.USER_DTREWARD_GET_SUCC);
		}
	}
	...
	public checkMonthCardRevival(): boolean {
		...
		if(monthCar1EndTime > nowTime) {
			if(monthCard1RevivalUseTime == 0 || alien.Utils.isTwoStampSameDay(monthCard1RevivalUseTime, nowTime) == false) {
				Alert.show("是否使用月卡复活机会？",0,(act)=>{
					if(act == "confirm") {
						let data = { optype: 10};
	...
	public checkMonthCardEndTime(): boolean {
		...
		let nowTime = server.getServerStamp();
		let difftimeCard1 = Number(alien.Utils.getTimeStampDiff(nowTime, monthcard1EndTime, "day"));
		let difftimeCard2 = Number(alien.Utils.getTimeStampDiff(nowTime, monthcard2EndTime, "day"));
	...
	public setFRechargeTodayHasGet():void{
        ...
        alien.Dispatcher.dispatch(EventNames.USER_FRREWARD_GET_SUCC);
	}
	...
	public addHornTalkRec(msgObj){
		...
		alien.Dispatcher.dispatch(EventNames.HORN_TALK_RECORDS_CHANGE,obj);
	}
	...
	public addSysTalkRec(msg){
		...
		*/
		alien.Dispatcher.dispatch(EventNames.HORN_TALK_RECORDS_CHANGE);
	}
	...
	public setNationalActInfo(_info):void{
		...
		alien.Dispatcher.dispatch(EventNames.NATIONAL_INFO_CHANGE);
	}
	...
	public getNationalInviteRew():number{
		if(this._nationalInfo && this._nationalInfo.inviteNum ){
			return GameConfig.getNationalInviteRewByNum(this._nationalInfo.inviteNum);
		}
		return 0;
	}
	...
	public updateActRed():void{
        alien.Dispatcher.dispatch(EventNames.ACT_RED_CHANGE);
	}
	...
	public getRoomRedWinCnt(roomId):number{
		...
		let _info = MainLogic.instance.selfData.getRoomWinCntInfo(roomId);
	...
	public getRoomNeedWinCntGetRed(roomId:number):number{
		...
		if(roomId == 1001 || roomId == 1002 || roomId == 8001 || roomId == 1006){
			...
			if(!_info){
				...
				return _nDefNum -(_info.cnt||0);
			}
		}
  		else if(this.wincnt && this.wincnt.length) {
			for(var i: number = 0;i < this.wincnt.length;i++) {
				if(this.wincnt[i].roomid == roomId) {
	...
	private _calcuateVipLevel():void{
		let vipCfg = GameConfig.getCfgByField("vip_config");
	...
	public hasGetTodayVipRew():boolean{
		...
		if(vipData && vipData.length >=2){
			let nTimeStamp = server.getServerStamp();
			let isSameDay = alien.Utils.isTwoStampSameDay(vipData[1]*1000,nTimeStamp);
			if(isSameDay){
				return true;
			}
		}
	...
	public setHasGetTodayVipRew():void{
		...
		let nTimeStamp = server.tsServer;
		if(len < 2){
			if(len<1){
	...
	public getTodayVipLucky():any{
		if(this["vipdata"].length >=3){
			...
			let curTime = server.getServerStamp();
			if(time<curTime){
				let hour = GameConfig.getCfgByField("custom.vipLuckyCfg.hourRange");
				let hours = hour.split("|");
				let bIn = alien.Utils.checkCurHourisInHourRange(Number(hours[0]),Number(hours[1]));
				if(time<=0){
					return {inTime:bIn};
				}
				
				let bOther = alien.Utils.checkTwoTimestampIsOtherDay(time,curTime);
				if(bOther){
					return {inTime:bIn};
				}else{
	 ...
	 * 是否能购买每日特惠
	 */
	public setCanBuyDailyGift(bCan:boolean):void{
		this["buyDailyGift"] = bCan;
	}
	...
	public shouldShowMonday():boolean{
		...
		let buyTimeStamp = 0;
		let curStamp = server.getServerStamp();
		...
		let sameDay = alien.Utils.isTwoStampSameDay(buyTimeStamp,curStamp);
		if(sameDay){
			return false;
		}*/
		return (this["buyDailyGift"]||false);
	}
...
//		this.losecnt = 0;
		...
		this.daytaskprogress = [0,0,0,0,0,0,0,0];
