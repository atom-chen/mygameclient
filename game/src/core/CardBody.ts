...
class CardBody extends egret.DisplayObjectContainer {
	...
	initData(pid):void {
		...
		if (pid > 0) {
			...
			let poker:any = this._poker = Utils.pid2poker(pid);
			if (poker.num > 13) {
				let joker:egret.Bitmap = new egret.Bitmap(RES.getRes('poker_' + poker.signColor + '_joker'));
				this.addChild(joker);

				let icon:egret.Bitmap = new egret.Bitmap(RES.getRes('poker_' + poker.signColor + '_joker_ddz_icon'));
				icon.x = 0;
				icon.y = 0;
				this.addChild(icon);
			} else {
	...
	set rollOver(value:boolean) {
		...
		if (this._rollOver != value) {
			...
			let card:Card = <Card>this.parent.parent;
			if(!this._rollOver && card.isGray()){
				return;
			}
			if(!!this._bg)
				this._bg.texture = RES.getRes('poker_broad' + (this._rollOver ? '_h' : ''));
		}
	}

	setBgGray(bGray):void{
		if(!!this._bg)
			this._bg.texture = RES.getRes('poker_broad' + (bGray ? '_h' : ''));
	}
