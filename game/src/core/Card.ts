...
class Card extends egret.DisplayObjectContainer {
	...
	private init():void {
		...
		this._container.addChild(this._body = new CardBody());
	 ...
	 * @returns {Card}
	 */
	static create(data):Card {
		return <Card>alien.ObjectPool.getObject('Card', data);
	}
	...
	static recycle(instance):void {
		alien.ObjectPool.recycleObject('Card', instance);
	}
