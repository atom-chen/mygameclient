...
var Publish = (function () {
    ...
    /**
     * 压缩js代码并修改copy index.html 
    */
    Publish.prototype.changeZipJs = function(_dir,overFunc){
        let _RootPath = _dir;
        let _libJs = path.join(_RootPath,'lib.min.js');
        let _mainJs = path.join(_RootPath,'main.min.js');
        var _zipPath = path.join(_RootPath,'game.zip');
        var _stream = fs.createWriteStream(_zipPath);
        ...
        archive.pipe(_stream);

        archive.append(fs.createReadStream(_libJs), { name: 'lib.min.js' });
        archive.append(fs.createReadStream(_mainJs), { name:'main.min.js'});
        archive.finalize();
    }
    ...
    * overFunc 文件解析完毕的回调
    */
    Publish.prototype.getFileLinesData = function(sFilePath,sContain,overFunc){
    ...
    * overFunc 修改完毕的回调
    */
    Publish.prototype.changeIndexHtml = function(sPath,overFunc){
        let _sPre = '<script egret=\"lib\" src=\"';
        let _sEnd = '\"></script>';
        let _sNewFile = "lib.min.js";
        let _sMainFile = "main.min.js";

        let _rootPath = path.dirname(sPath);
        let _sNewFilePath = path.join(_rootPath,_sNewFile);
        let _sNewCode = _sPre + _sNewFile + _sEnd;
        let _buf = "";

        this.getFileLinesData(sPath,_sPre,(_lines,_allData)=>{
            let _ctx = _allData;
            let _sLibFilePath = null;
            let _bHasWriteNew = false;
            let nTotal = _lines.length;
            let shiftNewLine = null;
            let _writeSteam = fs.createWriteStream(_sNewFilePath);
            let _self = this;
            let appendNewFile = function(_srcLibPath){
                let _readSteam = fs.createReadStream(_srcLibPath);
                _readSteam.on('data',function(chunk){
                    _writeSteam.write(chunk);
                })
                _readSteam.on('end',function(){
                    shiftNewLine(); 
                })
            }

            let onAllOver = function(){
                let _fd = fs.openSync(sPath,"w");
                fs.writeSync(_fd, _ctx);
                fs.closeSync(_fd);
                _writeSteam.end();
                FileUtil.remove(path.join(_rootPath,"libs"));
                FileUtil.remove(path.join(_rootPath,"promise"));
                _self.changeZipJs(_rootPath,overFunc);
            }

            shiftNewLine = function(){
                if(_lines.length < 1 ) {
                    onAllOver();
                    return;
                }
                ...
                let _nEndIdx = -1;
                let _nMainIdx = -1;
                _oneLine = _lines.shift();
                if(_oneLine){
                    _nPreIdx = _oneLine.indexOf(_sPre);
                    _nEndIdx = _oneLine.indexOf(_sEnd);
                    _nMainIdx = _oneLine.indexOf(_sMainFile);
                    if(_nPreIdx && _nEndIdx && (_nMainIdx == -1)){
                        if(!_bHasWriteNew){
                            _ctx =  _ctx.replace(_oneLine,"\t" +_sNewCode + "\n") ;
                            _bHasWriteNew = true;
                        }
                        else{
                            _ctx = _ctx.replace(_oneLine,"");
                        }
    
                        _sLibFilePath = _oneLine.substring(_nPreIdx + _sPre.length,_nEndIdx);
                        _sLibFilePath = path.join(_rootPath,_sLibFilePath);
                        
                        //console.log(_sLibFilePath);
                        appendNewFile(_sLibFilePath);
                    }
                    else{
                        shiftNewLine();
                    }
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (egret.args.runtime == "native") {
                            ...
                            this.changeIndexHtml(indexPath,function(){
                                FileUtil.copy(path.join(options.projectDir, "index_zip.html"), path.join(options.releaseDir, "index.html"));
                                FileUtil.copy(path.join(options.projectDir, "jszip.min.201710130900.js"), path.join(options.releaseDir, "jszip.min.201710130900.js"));
                            });
                        }
