...
- (IBAction)getCode:(id)sender {
	...
	if(mobile.length == 11){
		...
		[[WebService getInstance]getCode:_tiMobile.text callback:^(NSMutableDictionary *response) {
...
- (IBAction)confirm:(id)sender {
	...
	if(mobile.length == 0 || code.length == 0){
		...
		[[WebService getInstance]bindByMobile:mobile withCode:code withUID:[GameInfo getInstance].uid withCallback:^(NSMutableDictionary *response) {
			[_manager onBindResult:response withAlertTitle:@"手机号绑定失败" withUserInfo:@{@"phone": mobile, @"code": code}];
		}];
	}
}
