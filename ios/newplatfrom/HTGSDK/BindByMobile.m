...
- (IBAction)confirm:(id)sender {
	...
	if(mobile.length == 0 || code.length == 0){
        ...
        [[WebService getInstance]bindByMobileEncrpy:mobile withCode:code withUID:uid withCallback:^(NSMutableDictionary *response) {
            [_manager onBindResult:response withAlertTitle:@"手机号绑定失败" withUserInfo:@{@"phone": mobile, @"code": code} isDirect:NO];
        }];
	}
}
